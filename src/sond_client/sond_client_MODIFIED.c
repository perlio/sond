 ...
 *
 * Stellt sicher, dass Client authentifiziert ist.
 * Ruft Login-Callback auf falls kein Token vorhanden.
 *
 * Returns: TRUE wenn authentifiziert, FALSE bei Fehler
 */
static gboolean ensure_authenticated(SondClient *client, GError **error) {
    ...
    /* Nochmal prüfen ob jetzt Token vorhanden */
    if (!client->session_token || !client->username) {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_PERMISSION_DENIED,
 ...
 * handle_auth_error:
 *
 * Behandelt 401/403-Fehler. Versucht Re-Login.
 *
 * Returns: TRUE wenn Re-Login erfolgreich, FALSE sonst
 */
static gboolean handle_auth_error(SondClient *client, GError **error) {
    LOG_ERROR("Authentication failed (401/403) - session expired or invalid\n");
    ...
    /* Auth-Failed-Callback aufrufen (für UI-Benachrichtigung) */
    ...
    /* Prüfen ob Token jetzt vorhanden */
    if (!client->session_token || !client->username) {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_PERMISSION_DENIED,
...
gboolean sond_client_create_and_lock_node(SondClient *client,
    ...
    /* Authentifizierung sicherstellen (Lazy Login) */
    ...
    while (retry) {
        ...
        /* Auth-Fehler? (401 oder 403 oder 500 als Fallback für alten Server) */
        if (status == 401 || status == 403 || status == 500) {
            ...
            if (handle_auth_error(client, error)) {
                retry = TRUE;  /* Mit neuem Token nochmal versuchen */
                continue;
            }
            break;
        }

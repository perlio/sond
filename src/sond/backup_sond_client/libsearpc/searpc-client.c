...
searpc_client_transport_send(SearpcClient *client, const gchar *fcall_str,
	...
	return client->send(client->arg, fcall_str, fcall_len, ret_len);
}
...
fcall_to_str(const char *fname, int n_params, va_list args, gsize *len) {
	...
	for (; i < n_params; i++) {
		...
		if (strcmp(type, "int") == 0)
			json_array_append_new(array, json_integer((json_int_t) value));
...
void searpc_client_call(SearpcClient *client, const char *fname,
		const char *ret_type, GType gobject_type, void *ret_ptr, GError **error,
		int n_params, ...) {
...
int searpc_client_call__int(SearpcClient *client, const char *fname,
...
gint64 searpc_client_call__int64(SearpcClient *client, const char *fname,
		GError **error, int n_params, ...) {
	g_return_val_if_fail(fname != NULL, 0);
...
searpc_client_call__object(SearpcClient *client, const char *fname,
		GType object_type, GError **error, int n_params, ...) {
	g_return_val_if_fail(fname != NULL, NULL);
	...
	GObject *ret = searpc_client_fret__object(object_type, fret, ret_len,
			error);
	g_free(fstr);
	g_free(fret);
	return ret;
...
int searpc_client_generic_callback(char *retstr, size_t len, void *vdata,
		const char *errstr) {
	...
	if (errstr) {
		g_set_error(&error, DFT_DOMAIN, 500, "Transport error: %s", errstr);
		...
		if (strcmp(data->ret_type, "int64") == 0) {
			...
			result = (void*) searpc_client_fret__object(data->gtype, retstr,
					len, &error);
		} else if (strcmp(data->ret_type, "objlist") == 0) {
			result = (void*) searpc_client_fret__objlist(data->gtype, retstr,
					len, &error);
...
int searpc_client_async_call_v(SearpcClient *client, const char *fname,
		AsyncCallback callback, const gchar *ret_type, GType gtype,
		void *cbdata, int n_params, va_list args) {
	gsize len;
	char *fstr;
...
int searpc_client_async_call__int(SearpcClient *client, const char *fname,
		AsyncCallback callback, void *cbdata, int n_params, ...) {
...
int searpc_client_async_call__int64(SearpcClient *client, const char *fname,
		AsyncCallback callback, void *cbdata, int n_params, ...) {
	g_return_val_if_fail(fname != NULL, -1);
	...
	va_start(args, n_params);
	ret = searpc_client_async_call_v(client, fname, callback, "int64", 0,
			cbdata, n_params, args);
	va_end(args);
	return ret;
}

int searpc_client_async_call__string(SearpcClient *client, const char *fname,
		AsyncCallback callback, void *cbdata, int n_params, ...) {
	g_return_val_if_fail(fname != NULL, -1);
	...
	va_start(args, n_params);
	ret = searpc_client_async_call_v(client, fname, callback, "string", 0,
			cbdata, n_params, args);
	va_end(args);
	return ret;
}

int searpc_client_async_call__object(SearpcClient *client, const char *fname,
		AsyncCallback callback, GType object_type, void *cbdata, int n_params,
		...) {
	g_return_val_if_fail(fname != NULL, -1);
	...
	ret = searpc_client_async_call_v(client, fname, callback, "object",
			object_type, cbdata, n_params, args);
	va_end(args);
	return ret;
}

int searpc_client_async_call__objlist(SearpcClient *client, const char *fname,
		AsyncCallback callback, GType object_type, void *cbdata, int n_params,
		...) {
	g_return_val_if_fail(fname != NULL, -1);
...
int searpc_client_async_call__json(SearpcClient *client, const char *fname,
		AsyncCallback callback, void *cbdata, int n_params, ...) {
	g_return_val_if_fail(fname != NULL, -1);
	...
	va_start(args, n_params);
	ret = searpc_client_async_call_v(client, fname, callback, "json", 0, cbdata,
			n_params, args);
	va_end(args);
	return ret;
}
 ...
 */
static int handle_ret_common(char *data, size_t len, json_t **object,
		GError **error) {
	int err_code;
	const char *err_msg;
	json_error_t jerror;
	...
	*object = json_loadb(data, len, 0, &jerror);
	...
	if (json_object_get(*object, "err_code")) {
		...
		g_set_error(error, DFT_DOMAIN, err_code, "%s", err_msg);
		json_decref(*object);
		return -1;
	}
...
gint64 searpc_client_fret__int64(char *data, size_t len, GError **error) {
	json_t *object = NULL;
	gint64 ret;

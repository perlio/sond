...
static void func_item_free(FuncItem *item) {
	g_free(item->fname);
	g_free(item);
}

static void marshal_item_free(MarshalItem *item) {
...
int searpc_create_service(const char *svc_name) {
	...
	service->func_table = g_hash_table_new_full(g_str_hash, g_str_equal, NULL,
			(GDestroyNotify) func_item_free);
...
static void service_free(SearpcService *service) {
	g_free(service->name);
...
searpc_marshal_set_ret_common(json_t *object, gsize *len, GError *error) {
	...
	if (error) {
		json_object_set_new(object, "err_code",
				json_integer((json_int_t) error->code));
...
void searpc_server_init(RegisterMarshalFunc register_func) {
	marshal_table = g_hash_table_new_full(g_str_hash, g_str_equal, NULL,
			(GDestroyNotify) marshal_item_free);
	service_table = g_hash_table_new_full(g_str_hash, g_str_equal, NULL,
			(GDestroyNotify) service_free);
...
gboolean searpc_server_register_function(const char *svc_name, void *func,
		const gchar *fname, gchar *signature) {
	SearpcService *service;
	FuncItem *item;
	MarshalItem *mitem;

	g_assert(
			svc_name != NULL && func != NULL && fname != NULL
					&& signature != NULL);
...
char*
searpc_server_call_function(const char *svc_name, gchar *func, gsize len,
		gsize *ret_len) {
	SearpcService *service;
	json_t *array;
	char *ret;

...
static void sond_client_akte_close(SondClientAkte *sond_client_akte) {
	GtkWidget *window = NULL;

	g_ptr_array_remove_fast(sond_client_akte->sond_client->arr_children_windows,
			sond_client_akte);
...
static void sond_client_akte_button_abbrechen_clicked(GtkButton *button,
		gpointer data) {
...
static gboolean sond_client_akte_delete_event(GtkWindow *window,
...
static void sond_client_akte_button_ok_clicked(GtkButton *button, gpointer data) {
	gchar *params = NULL;
	gchar *resp = NULL;
	...
	if (g_strcmp0(sond_client_akte->sond_akte->aktenrubrum,
			gtk_entry_get_text(
					GTK_ENTRY(sond_client_akte->entry_aktenrubrum)))) { //daß ins entry nichts eingetragen ist (""), wurde bereits zuvor abgefragt
		changed = TRUE;
	...
	if (g_strcmp0(sond_client_akte->sond_akte->aktenkurzbez,
			gtk_entry_get_text(
					GTK_ENTRY(sond_client_akte->entry_aktenkurzbez)))) {
		changed = TRUE;
			...
			(((sond_client_akte->sond_akte->arr_leben->len % 3) == 0) == //Abgelegt == TRUE
					gtk_widget_get_sensitive(
							gtk_widget_get_parent(
									sond_client_akte->entry_aktenrubrum)))) //Akte editable
			{
		changed = TRUE;

		//Marker ans Ende des Lebens-array setzen
		g_ptr_array_add(sond_client_akte->sond_akte->arr_leben,
				GUINT_TO_POINTER(1));
	}
	...
	if (changed)
		do {
			...
			if (!resp) {
				...
				if (strlen(resp) > 7) {
					title = g_strconcat("Schreibschutz für ", resp + 7,
							" besteht", NULL);
					second = g_strdup("Trotzdem speichern?");
				} else {
					title = g_strdup(
							"Zwischenzeitliche Änderungen durch anderen user");
					second = g_strdup("Änderungen überschreiben?");
				}

				rc = abfrage_frage(sond_client_akte->window, title, second,
						NULL);
				g_free(title);
				g_free(second);
				if (rc == GTK_RESPONSE_NO) {
					...
					rc = sond_client_get_lock(sond_client_akte->sond_client,
							sond_client_akte->sond_akte->ID_entity, &error);
					if (rc) {
						display_message(sond_client_akte->window,
...
static void sond_client_akte_set_activ(SondClientAkte *sond_client_akte,
		gboolean activ) {
	gtk_widget_set_sensitive(
			gtk_widget_get_parent(sond_client_akte->entry_aktenrubrum), activ);
	gtk_widget_set_sensitive(
			gtk_widget_get_parent(sond_client_akte->entry_aktenkurzbez), activ);

	if (activ)
		gtk_button_set_label(GTK_BUTTON(sond_client_akte->button_activ),
				"Ablegen");
	else
		gtk_button_set_label(GTK_BUTTON(sond_client_akte->button_activ),
				"Reaktivieren");
...
static void sond_client_akte_loaded(SondClientAkte *sond_client_akte,
		gboolean unlocked, gboolean neu) {
	gtk_widget_set_sensitive(sond_client_akte->button_neu, FALSE);
	gtk_widget_set_sensitive(sond_client_akte->entry_reg_nr, FALSE);

	sond_client_akte_set_activ(sond_client_akte,
			neu
					|| (unlocked
							&& (sond_client_akte->sond_akte->arr_leben->len % 3)));

	gtk_widget_set_sensitive(sond_client_akte->button_unlock, !unlocked);

	gtk_widget_set_sensitive(sond_client_akte->button_activ,
			(!neu) && unlocked);
...
static void sond_client_akte_button_unlock_clicked(GtkButton *button,
		gpointer data) {
	gint rc = 0;
	GError *error = NULL;
	...
	if (rc) {
		display_message(sond_client_akte->window,
				"Akte konnte nicht entsperrt werden\n\n", error->message, NULL);
		g_error_free(error);
...
static gint sond_client_akte_holen(SondClientAkte *sond_client_akte,
		gint reg_nr, gint reg_jahr, gchar **user, GError **error) {
	gchar *resp = NULL;
	...
	if (g_str_has_prefix(resp, "LOCKED")) {
		...
		if (user)
			*user = g_strndup(resp + 6,
					strlen(resp) - 6 - strlen(json_string) - 1);
	} else
		json_string = resp;
...
static void sond_client_akte_load(SondClientAkte *sond_client_akte, gint reg_nr,
		gint reg_jahr) {
	gchar *user = FALSE;
	GError *error = NULL;
	gint rc = 0;
	...
	for (gint i = 0;
			i < sond_client_akte->sond_client->arr_children_windows->len; i++) {
		SondClientAny *sond_client_any = NULL;

		sond_client_any = g_ptr_array_index(
				sond_client_akte->sond_client->arr_children_windows, i);

		if (sond_client_any->type == SOND_CLIENT_TYPE_AKTE
				&& ((SondClientAkte*) sond_client_any)->sond_akte
				&& ((SondClientAkte*) sond_client_any)->sond_akte->reg_nr
						== reg_nr
				&& ((SondClientAkte*) sond_client_any)->sond_akte->reg_jahr
						== reg_jahr) {
			sond_client_akte_close(sond_client_akte);
	...
	rc = sond_client_akte_holen(sond_client_akte, reg_nr, reg_jahr, &user,
			&error);
	if (rc == -1) {
		display_message(sond_client_akte->window,
				"Akte kann nicht geladen werden\n\n", error->message, NULL);
		g_error_free(error);
		...
		return; //Fenster bleibt geöffnet; je nach Fehler kann man es nochmal versuchen
			...
			{
		gint res = 0;
		res = abfrage_frage(sond_client_akte->window, "Akte existiert nicht",
				"Erstellen?", NULL);
		if (res != GTK_RESPONSE_YES)
			gtk_entry_set_text(GTK_ENTRY(sond_client_akte->entry_reg_nr), "");
		else
	...
	for (gint i = 0; i < sond_client_akte->sond_akte->arr_leben->len; i += 3) {
		...
		val = GPOINTER_TO_UINT(
				g_ptr_array_index( sond_client_akte->sond_akte->arr_leben, i ));
		s_tm = localtime(&val);
		strftime(time_text, 64, "%d. %b %Y", s_tm);
		text = g_strdup_printf("Anlage: %s", time_text);
		text_label = add_string( NULL, text);

		if (i + 1 >= sond_client_akte->sond_akte->arr_leben->len)
			break;

		val =
				GPOINTER_TO_UINT(
						g_ptr_array_index( sond_client_akte->sond_akte->arr_leben, i + 1 ));
		s_tm = localtime(&val);
		strftime(time_text, 64, "%d. %b %Y", s_tm);
		...
		val =
				GPOINTER_TO_UINT(
						g_ptr_array_index( sond_client_akte->sond_akte->arr_leben, i + 2 ));
		text = g_strdup_printf(" - Ablagenr: %lld", val);
		text_label = add_string(text_label, text);
	}

	gtk_label_set_text(GTK_LABEL(sond_client_akte->label_popover_hist),
			text_label);
	g_free(text_label);

	if (user)
		display_message(sond_client_akte->window, "Akte ist zur "
...
static void sond_client_akte_auswahlfenster_row_activated(GtkListBox *listbox,
...
static gint sond_client_akte_auswahlfenster(SondClientAkte *sond_client_akte,
	...
	window = result_listbox_new(GTK_WINDOW(sond_client_akte->window),
			"Akte auswählen", GTK_SELECTION_BROWSE);

	listbox = g_object_get_data(G_OBJECT(window), "listbox");
	g_signal_connect(listbox, "row-activated",
			G_CALLBACK(sond_client_akte_auswahlfenster_row_activated),
			sond_client_akte);
	...
	if (!JSON_NODE_HOLDS_ARRAY(jnode)) {
		if (error)
			*error = g_error_new( SOND_CLIENT_ERROR, 0,
...
//        ID_entity = json_object_get_int_member( jobject, "ID_entity" );
		...
		label_text = g_strdup_printf(" %4i/%i  %s  (%s)", reg_nr,
				reg_jahr % 100, aktenrubrum, aktenkurzbez);
...
static void sond_client_akte_button_activ_clicked(GtkButton *button,
		gpointer data) {
	gboolean active = FALSE;

	SondClientAkte *sond_client_akte = (SondClientAkte*) data;

	active = gtk_widget_get_sensitive(
			gtk_widget_get_parent(sond_client_akte->entry_aktenrubrum));
...
static void sond_client_akte_entry_reg_nr_activate(GtkEntry *entry,
		gpointer data) {
	SondClientAkte *sond_client_akte = (SondClientAkte*) data;

	if (strlen(gtk_entry_get_text(entry)) < 3) {
		display_message(sond_client_akte->window, "Mindestens drei Zeichen",
				NULL);
	...
	if (sond_client_misc_regnr_wohlgeformt(gtk_entry_get_text(entry))) {
		...
		sond_client_misc_parse_regnr(gtk_entry_get_text(entry), &reg_nr,
				&reg_jahr);
		sond_client_akte_load(sond_client_akte, reg_nr, reg_jahr);
...
static void sond_client_akte_button_neu_clicked(GtkButton *button,
		gpointer data) {
	SondClientAkte *sond_client_akte = (SondClientAkte*) data;
...
void sond_client_akte_init(GtkButton *button, gpointer data) {
	...
	gtk_window_set_titlebar(GTK_WINDOW(sond_client_akte->window),
			akten_headerbar);
	...
	if ((sond_client_akte->sond_client->reg_jahr_akt)
			&& (sond_client_akte->sond_client->reg_nr_akt)) {
		gchar *text_regnr = g_strdup_printf("%i/%i",
				sond_client_akte->sond_client->reg_nr_akt,
	...
	gtk_container_add(GTK_CONTAINER(frame_regnr),
			sond_client_akte->entry_reg_nr);
	...
	gtk_header_bar_pack_end(GTK_HEADER_BAR(akten_headerbar),
			sond_client_akte->button_unlock);
	gtk_widget_set_sensitive(sond_client_akte->button_unlock, FALSE);

	//Button Neue Akte
	sond_client_akte->button_neu = gtk_button_new_with_label("Neue Akte");
	gtk_header_bar_pack_start(GTK_HEADER_BAR(akten_headerbar),
			sond_client_akte->button_neu);

	//Entry "Aktenrubrum"
	GtkWidget *frame_aktenrubrum = gtk_frame_new("Rubrum");
	sond_client_akte->entry_aktenrubrum = gtk_entry_new();
	gtk_container_add(GTK_CONTAINER(frame_aktenrubrum),
			sond_client_akte->entry_aktenrubrum);
	...
	gtk_container_add(GTK_CONTAINER(frame_aktenkurzbez),
			sond_client_akte->entry_aktenkurzbez);
	...
	g_signal_connect(sond_client_akte->button_activ, "clicked",
			G_CALLBACK(sond_client_akte_button_activ_clicked),
			sond_client_akte);
	...
	gtk_container_add(GTK_CONTAINER(popover_hist),
			sond_client_akte->label_popover_hist);
	...
	g_signal_connect_swapped(sond_client_akte->button_activ,
			"enter-notify-event", G_CALLBACK(gtk_popover_popup), popover_hist);
	g_signal_connect_swapped(sond_client_akte->button_activ,
	...
	sond_client_akte->button_ok = gtk_button_new_with_label("OK");
	...
	g_signal_connect(sond_client_akte->entry_reg_nr, "activate",
			G_CALLBACK(sond_client_akte_entry_reg_nr_activate),
			sond_client_akte);

	g_signal_connect(sond_client_akte->button_unlock, "clicked",
			G_CALLBACK(sond_client_akte_button_unlock_clicked),
			sond_client_akte);
	...
	g_ptr_array_add(sond_client_akte->sond_client->arr_children_windows,
			sond_client_akte);

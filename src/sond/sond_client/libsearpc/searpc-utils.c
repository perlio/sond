...
static gboolean json_deserialize_pspec(GValue *value, GParamSpec *pspec,
		json_t *node) {
...
GObject* json_gobject_deserialize(GType gtype, json_t *object) {
	...
	construct_params = g_array_sized_new(FALSE, FALSE, sizeof(GParameter),
			n_members);
	head = json_object_iter(object);

	for (member = head; member;
			member = json_object_iter_next(object, member)) {
		GParamSpec *pspec;
		...
		if (json_deserialize_pspec(&param.value, pspec, val)) {
			...
			g_warning(
					"Failed to deserialize \"%s\" property of type \"%s\" for an object of type \"%s\"",
					pspec->name, g_type_name(G_VALUE_TYPE(&param.value)),
					g_type_name(gtype));
	}

	ret = g_object_newv(gtype, construct_params->len,

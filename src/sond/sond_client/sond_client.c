...
gchar*
sond_client_send_and_read(SondClient *sond_client, const gchar *command,
		const gchar *params, GError **error) {
...
//    g_socket_client_set_tls( client, TRUE );
	...
	connection = g_socket_client_connect_to_host(client,
			sond_client->server_host, sond_client->server_port, NULL, error);
	if (*error) {
...
gint sond_client_unlock(SondClient *sond_client, gint ID_entity, GError **error) {
	gchar *resp = NULL;
	gchar *params = NULL;
...
gint sond_client_get_lock(SondClient *sond_client, gint ID_entity,
		GError **error) {
	gchar *params = NULL;
	gchar *resp = NULL;
...
static gboolean sond_client_close(GtkWidget *app_window, GdkEvent *event,
		gpointer data) {
...
void sond_client_quit(SondClient *sond_client) {
...
static char* b64encode(const char *input) {
	char buf[32767] = { 0 };
	DWORD retlen = 32767;
	CryptBinaryToStringA((BYTE*) input, strlen(input),
			CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, buf, &retlen);
	return strdup(buf);
}
#endif //__WIN32

static gint sond_client_init_rpc_client(SondClient *sond_client, GError **error) {
	...
	sond_client->searpc_client = searpc_client_with_named_pipe_transport(
			searpc_named_pipe_client, "seafile-rpcserver");
...
static gchar*
sond_client_create_hash(const gchar *password, const gchar *salt_b64,
		GError **error) {
...
static gboolean sond_client_ping_server(SondClient *sond_client, GError **error) {
...
static gint sond_client_get_creds(SondClient *sond_client, GError **error) {
	gint ret = 0;
	gchar user[256] = { 0 };

	GtkWidget *dialog = gtk_dialog_new_with_buttons("Password",
			GTK_WINDOW(sond_client->app_window),
	...
	if (res == GTK_RESPONSE_OK) {
		if (g_strcmp0(user, sond_client->user)) //anderer user als gespeichert
			...
			hash_b64 = sond_client_create_hash(sond_client->password, salt_b64,
					error);
			if (!hash_b64) {
				g_free(salt_b64);
				g_prefix_error(error, "%s\n", __func__);
			...
			conf_path = g_build_filename(sond_client->base_dir,
					"SondClient.conf", NULL);
		...
		} else //user ist gepspeicherter user
		{
			...
			if (g_strcmp0(hash_b64, sond_client->password_hash)) {
				*error = g_error_new( SOND_CLIENT_ERROR,
...
static gint sond_client_read_conf(SondClient *sond_client, GError **error) {
	GKeyFile *key_file = NULL;
	gchar *conf_path = NULL;
	gboolean success = FALSE;
	gchar *hash = NULL;
	...
	conf_path = g_build_filename(sond_client->base_dir, "SondClient.conf",
			NULL);
	success = g_key_file_load_from_file(key_file, conf_path,
	...
	if (user && g_strcmp0(user, "")) {
		...
		if (hash && g_strcmp0(hash, "")) {
			salt = g_key_file_get_string(key_file, "CREDS", "password_salt",
					NULL);
	...
	sond_client->server_host = g_key_file_get_string(key_file, "SERVER", "host",
			error);
	if (*error) {
		g_prefix_error(error, "%s\n", __func__);
		return -1;
	...
	sond_client->server_port = (guint16) g_key_file_get_uint64(key_file,
			"SERVER", "port", error);
	if (*error) {
		g_prefix_error(error, "%s\n", __func__);
		return -1;
	...
	sond_client->seafile_root = g_key_file_get_string(key_file, "SEAFILE",
			"root", error);
	if (*error) {
		g_prefix_error(error, "%s\n", __func__);
		return -1;
	...
	sond_client->seadrive_root = g_key_file_get_string(key_file, "SEADRIVE",
			"root", error);
	if (*error) {
		g_prefix_error(error, "%s\n", __func__);
		return -1;
...
static void sond_client_init_app_window(GtkApplication *app,
	...
	g_signal_connect(button_akte, "clicked", G_CALLBACK(sond_client_akte_init),
			sond_client);

	return;
}

static void sond_client_init(GtkApplication *app, SondClient *sond_client) {
	gint rc = 0;
	GError *error = NULL;
...
gint main(int argc, char **argv) {
	...
	app = gtk_application_new("de.rubarth-krieger.sond_client",

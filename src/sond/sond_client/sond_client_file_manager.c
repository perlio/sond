...
static void sond_client_file_manager_set_headerbar(GtkWidget *fm_window,
...
static gboolean sond_client_file_manager_create(SondClient *sond_client,
		gint jahr, gint regnr, GError **error) {
...
void sond_client_file_manager_entry_activate(GtkEntry *entry, gpointer data) {
	gint regnr = 0;
	gint jahr = 0;
	...
	if (sond_client_misc_regnr_wohlgeformt(gtk_entry_get_text(entry))) {
		...
		if (!sond_client_file_manager_create(sond_client, jahr, regnr,
				&error)) {

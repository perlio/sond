...
G_DEFINE_TYPE_WITH_PRIVATE(SondDatabaseEntity, sond_database_entity,
		GTK_TYPE_BOX)

static void sond_database_entity_class_init(SondDatabaseEntityClass *klass) {
...
static void sond_database_entity_init(SondDatabaseEntity *self) {
	...
	gtk_box_pack_start(GTK_BOX(priv->box_entity), frame_ID_label, FALSE, FALSE,
			0);
	...
	gtk_box_pack_start(GTK_BOX(box_padding), priv->box_properties, FALSE, FALSE,
			0);
...
sond_database_entity_new(void) {
	...
	gtk_orientable_set_orientation(GTK_ORIENTABLE(sde),
			GTK_ORIENTATION_VERTICAL);

	return GTK_WIDGET(sde);
}

gint sond_database_entity_load(SondDatabaseEntity *sde, gpointer database,
...
//    rc = sond_database_get_label_for_ID_label( database, ID_label, &label, errmsg );
	...
	for (gint i = 0; i < arr_properties->len; i++) {
		...
		//in sde einfügen
		gtk_box_pack_start(GTK_BOX(priv->box_properties), GTK_WIDGET(sdp),
				FALSE, FALSE, 0);
	}
...
sond_database_entity_load_new(gpointer database, gint ID, gchar **errmsg) {
	...
	rc = sond_database_entity_load(SOND_DATABASE_ENTITY(sde), database, ID,
			errmsg);
	if (rc) {
		g_object_unref(sde);
		ERROR_S_VAL(NULL)
...
sond_database_entity_get_prop_box(SondDatabaseEntity *sde) {
	SondDatabaseEntityPrivate *priv = sond_database_entity_get_instance_private(
			sde);

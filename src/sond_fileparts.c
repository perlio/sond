...
{
    size_t la, lb;
    const char *sa = pdf_to_str_buf(ctx, a, &la);
    const char *sb = pdf_to_str_buf(ctx, b, &lb);
    ...
    fz_try(ctx)
    {
        ...
        if (kids)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                size_t l1, l2;
                const char *low  = pdf_to_str_buf(ctx, pdf_array_get(ctx, kid_limits, 0), &l1);
                const char *high = pdf_to_str_buf(ctx, pdf_array_get(ctx, kid_limits, 1), &l2);

...
static void sojus_init_monitor_changed(GFileMonitor *monitor, GFile *file,
		GFile *other_file, GFileMonitorEvent type, gpointer data) {
	gchar *str_file = NULL;
...
//    const gchar* str_type = NULL;
	...
	if (file)
		str_file = g_file_get_basename(file);
	if (other_file)
		str_other_file = g_file_get_basename(other_file);

	if (type == G_FILE_MONITOR_EVENT_CREATED) {
		const gchar *last_inserted = NULL;

		last_inserted = (const gchar*) g_object_get_data(G_OBJECT(monitor),
				"last-inserted");

		//Bei SMB-Zugriff kommt es zur doppeltenAuslsung des created-Signals
		//war als letztes schon created - Doppel
		if (!g_strcmp0(last_inserted, str_file))
			g_object_set_data(G_OBJECT(monitor), "last-inserted", NULL);
		//wirklich neu
		else
			g_ptr_array_add(sojus->arr_dirs, g_strdup(str_file));
	} else if (type == G_FILE_MONITOR_EVENT_DELETED
			|| type == G_FILE_MONITOR_EVENT_RENAMED) {
		gint i = 0;

		//index ermitteln
		for (i = 0; i < sojus->arr_dirs->len; i++)
			if (!g_strcmp0(g_ptr_array_index(sojus->arr_dirs, i), str_file))
				break;
...
static void sojus_init_load_dirs(Sojus *sojus) {
	...
	if (!success) {
		display_message(sojus->app_window,
				"Sojus.conf konnte nicht gelesen werden:\n", error->message,
				NULL);
		g_error_free(error);
	...
	if (error) {
		display_message(sojus->app_window,
				"Sojus root-dir konnte nicht ermittelt werden:\n",
				error->message, NULL);
		g_error_free(error);
		g_key_file_free(key_file);
	...
	while (1) {
		...
		if (!g_file_enumerator_iterate(enumer, NULL, &file_child, NULL,
				&error)) {
			display_message(sojus->app_window,
					"Bei Aufruf g_file_enumerator_iterate:\n", error->message,
					NULL);
			g_error_free(error);
			g_object_unref(enumer);
			g_object_unref(root);

			g_signal_emit_by_name(sojus->app_window, "delete-event", NULL,
					&ret);
		}

		if (file_child) //es gibt noch Datei in Verzeichnis
		{
			GFileType type = g_file_query_file_type(file_child,
					G_FILE_QUERY_INFO_NONE, NULL);
			if (type == G_FILE_TYPE_DIRECTORY)
				g_ptr_array_add(sojus->arr_dirs,
	...
	//Monitor f�r Dir
	sojus->monitor = g_file_monitor_directory(root, G_FILE_MONITOR_WATCH_MOVES,
			NULL, &error);
	g_object_unref(root);
	if (!(sojus->monitor)) {
		display_message(sojus->app_window,
				"Root-Dir kann nicht �berwacht werden:\n", error->message,
				NULL);
		g_error_free(error);
	...
	g_signal_connect(sojus->monitor, "changed",
			G_CALLBACK(sojus_init_monitor_changed), sojus);

	return;
}

static void log_init(Sojus *sojus) {
	gchar *logfile = NULL;
	FILE *file = NULL;
	FILE *file_tmp = NULL;
...
static void sojus_init_entry_activated(GtkWidget *entry, gpointer data) {
...
static gboolean cb_desktop_delete_event(GtkWidget *app_window, GdkEvent *event,
		gpointer data) {
	Sojus *sojus = (Sojus*) data;
...
static void sojus_init_app_window(Sojus *sojus) {
	/*
	 **  Widgets erzeugen  */
	//app-window
	...
	//Signal f�r App-Fenster schlie�en

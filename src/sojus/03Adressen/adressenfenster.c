...
gboolean cb_window_adr_delete_event(GtkWidget *window, GdkEvent *event,
		gpointer user_data) {
	...
	if (gtk_widget_get_sensitive(button_speichern)) {
		...
		if (rc == GTK_RESPONSE_YES)
			adresse_speichern(window);
	}

	return FALSE;
}

void cb_button_abbrechen_adr_clicked(GtkButton *button, gpointer user_data) {
	...
	if (gtk_widget_get_sensitive(button_speichern)) {
		gint rc = 0;
		rc = ask_question(adressen_window, "Änderungen speichern?", "Ja",
				"Nein");
...
void cb_button_speichern_adr_clicked(GtkButton *button,
...
void cb_button_ok_adr_clicked(GtkButton *button, gpointer user_data) {
	GtkWidget *adressen_window = (GtkWidget*) user_data;
	GtkWidget *button_speichern = g_object_get_data(G_OBJECT(adressen_window),
			"button_speichern");
...
void cb_adresse_geaendert(gpointer window) {
	widgets_adresse_geaendert((GObject*) window, TRUE);

	return;
}

void cb_button_neue_adresse_clicked(GtkButton *button, gpointer user_data) {
	GtkWidget *adressen_window = (GtkWidget*) user_data;
...
void cb_entry_adressnr_activate(GtkEntry *entry, gpointer user_data) {
	GtkWidget *adressen_window = (GtkWidget*) user_data;
	...
	if (ret)
		adressenfenster_fuellen(adressen_window, sojus->adressnr_akt);
	else {
...
adressenfenster_oeffnen(Sojus *sojus) {
	...
	gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(textview_bemerkungen),
			GTK_WRAP_WORD);
	...
	g_object_set_data(G_OBJECT(adressen_window), "entry_adressnr",
			entry_adressnr);
	g_object_set_data(G_OBJECT(adressen_window), "button_neue_adresse",
			button_neue_adresse);

	g_object_set_data(G_OBJECT(adressen_window), "entry_adresszeile1",
			entry_adresszeile1);
	g_object_set_data(G_OBJECT(adressen_window), "entry_titel", entry_titel);
	g_object_set_data(G_OBJECT(adressen_window), "entry_vorname",
			entry_vorname);
	g_object_set_data(G_OBJECT(adressen_window), "entry_name", entry_name);
	g_object_set_data(G_OBJECT(adressen_window), "entry_adresszusatz",
			entry_adresszusatz);
	g_object_set_data(G_OBJECT(adressen_window), "entry_strasse",
			entry_strasse);
	...
	g_object_set_data(G_OBJECT(adressen_window), "entry_telefon1",
			entry_telefon1);
	g_object_set_data(G_OBJECT(adressen_window), "entry_telefon2",
			entry_telefon2);
	g_object_set_data(G_OBJECT(adressen_window), "entry_telefon3",
			entry_telefon3);
	...
	g_object_set_data(G_OBJECT(adressen_window), "entry_homepage",
			entry_homepage);
	...
	g_object_set_data(G_OBJECT(adressen_window), "textview_bemerkungen",
			textview_bemerkungen);

	g_object_set_data(G_OBJECT(adressen_window), "button_ok", button_ok);
	g_object_set_data(G_OBJECT(adressen_window), "button_speichern",
			button_speichern);
	...
	g_signal_connect(button_speichern, "clicked",
			G_CALLBACK(cb_button_speichern_adr_clicked),
			(gpointer ) adressen_window);

	//button abbrechen
	g_signal_connect(button_abbrechen, "clicked",
			G_CALLBACK(cb_button_abbrechen_adr_clicked),
			(gpointer ) adressen_window);
...
void adressenfenster_fuellen(GtkWidget *adressen_window, gint adressnr) {
	if (adressnr > 0) {
		/*
		 **  Adresse holen  */
		...
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_adressnr" )),
				text);
		g_free(text);
		...
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_adresszeile1" )),
				adresse->adresszeile1);

		//entry_titel
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_titel" )),
				adresse->titel);

		//entry_vorname
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_vorname" )),
				adresse->vorname);

		//entry_name
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_name" )),
				adresse->name);

		//entry_adresszusatz
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_adresszusatz" )),
				adresse->adresszusatz);

		//entry_strasse
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_strasse" )),
				adresse->strasse);

		//entry_hausnr
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_hausnr" )),
				adresse->hausnr);

		//entry_plz
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_plz" )),
				adresse->plz);

		//entry_ort
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_ort" )),
				adresse->ort);

		//entry_land
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_land" )),
				adresse->land);

		//entry_telefon1
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_telefon1" )),
				adresse->telefon1);

		//entry_telefon2
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_telefon2" )),
				adresse->telefon2);

		//entry_telefon3
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_telefon3" )),
				adresse->telefon3);

		//entry_fax
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_fax" )),
				adresse->fax);

		//entry_email
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_email" )),
				adresse->email);

		//entry_homepage
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_homepage" )),
				adresse->homepage);

		//entry_iban
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_iban" )),
				adresse->iban);

		//entry_bic
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_bic" )),
				adresse->bic);

		//entry_anrede
		gtk_entry_set_text(
				GTK_ENTRY(
						g_object_get_data( G_OBJECT(adressen_window), "entry_anrede" )),
				adresse->anrede);
		...
		GtkTextBuffer *textview_buffer_bemerkungen = gtk_text_view_get_buffer(
				g_object_get_data(G_OBJECT(adressen_window),
						"textview_bemerkungen"));
		gtk_text_buffer_set_text(textview_buffer_bemerkungen,
				adresse->bemerkungen, -1);
		/*
		 **  Adresse freigeben  */

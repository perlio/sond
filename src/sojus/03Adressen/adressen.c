...
void adresse_free(Adresse *adresse) {
...
void adresse_speichern(GtkWidget *adressen_window) {
	Sojus *sojus = (Sojus*) g_object_get_data(G_OBJECT(adressen_window),
			"sojus");
	/*
	...
	if (g_strcmp0(adressnr_text, "- neu -"))
		adressnr = atoi(adressnr_text);
	else {
	...
	GtkWidget *entry_adresszeile1 = g_object_get_data(G_OBJECT(adressen_window),
			"entry_adresszeile1");
	const gchar *adresszeile1 = gtk_entry_get_text(
	...
	GtkWidget *entry_adresszusatz = g_object_get_data(G_OBJECT(adressen_window),
			"entry_adresszusatz");
	const gchar *adresszusatz = gtk_entry_get_text(
	...
	GtkTextView *textview_bemerkungen = g_object_get_data(
			G_OBJECT(adressen_window), "textview_bemerkungen");
	...
	sql =
			g_strdup_printf(
					"UPDATE Adressen SET `Adresszeile1`='%s', "
							...
							"WHERE AdressNr=%i;", adresszeile1, titel, vorname,
					name, adresszusatz, strasse, hausnr, plz, ort, land,
					telefon1, telefon2, telefon3, fax, email, homepage, iban,
					bic, anrede, bemerkungen, adressnr);

...
gboolean naechste_adressnr(Sojus *sojus) {
	gint adressnr = 0;
	gchar *sql = NULL;
	gint rc = 0;
...
gboolean adressnr_ist_wohlgeformt(gchar *text) {
	gint pos = 0;
	while ((*(text + pos)) != 0) {
...
gboolean adressnr_existiert(GtkWidget *window, MYSQL *con, gint adressnr) {
	gchar *sql = NULL;
	gint rc = 0;
...
gboolean parse_adressnr(GtkWidget *window, gchar *text) {
	...
	if (rows->len > 0) {
		GtkDialog *dialog = auswahl_dialog_oeffnen(window, "Adresse auswählen",
				"   Adressnr.  Name, Vorname");

		for (gint i = 0; i < rows->len; i++) {
			row = g_ptr_array_index(rows, i);
			gchar *text = g_strdup_printf("%6s   %s, %s", row[0], row[4],
					row[3]);

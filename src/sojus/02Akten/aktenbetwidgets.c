...
void
cb_button_verwerfen_clicked( GtkButton* button, gpointer user_data )
{
    ...
    Aktenbet* aktenbet = (Aktenbet*) g_object_get_data( G_OBJECT(row), "aktenbet" );
    ...
    if ( !(aktenbet->adressnr) )
    {
        ...
        while ( (index >= 0) && !gtk_list_box_get_row_at_index(
                GTK_LIST_BOX(listbox_aktenbet), index ) ) index--;

        if ( index >= 0 ) gtk_list_box_select_row( GTK_LIST_BOX(listbox_aktenbet),
                gtk_list_box_get_row_at_index( GTK_LIST_BOX(listbox_aktenbet),
                index ) );
        else widgets_aktenbeteiligte_vorhanden( G_OBJECT(akten_window), FALSE );
    }
    else aktenbetwidgets_fuellen( akten_window, aktenbet );

    gpointer geaendert = g_object_get_data( G_OBJECT(akten_window),
            "geaendert" );

    if ( geaendert ) widgets_akte_geaendert( G_OBJECT(akten_window), TRUE );
...
void
cb_button_uebernehmen_clicked( GtkButton* button, gpointer user_data )
{
    GtkWidget* akten_window = (GtkWidget*) user_data;
    GtkWidget* listbox_aktenbet = (GtkWidget*) g_object_get_data(
            G_OBJECT(akten_window), "listbox_aktenbet" );
...
void
cb_adressenfenster_destroy( GtkWidget* widget, gpointer user_data )
{
    GtkWidget* akten_window = (GtkWidget*) user_data;
...
void
cb_button_aktenbet_neue_adresse_clicked( GtkButton* button, gpointer user_data )
{
    GtkWidget* akten_window = (GtkWidget*) user_data;
    Sojus* sojus = (Sojus*) g_object_get_data( G_OBJECT(akten_window), "sojus" );
...
void
cb_entry_aktenbet_adressnr_activate( GtkEntry* entry, gpointer user_data )
{
    GtkWidget* akten_window = (GtkWidget*) user_data;
    Sojus* sojus = (Sojus*) g_object_get_data( G_OBJECT(akten_window), "sojus" );
    ...
    if ( ret ) entry_text = g_strdup_printf( "%i",
            sojus->adressnr_akt );
    ...
    if ( ret )
    {
        ...
        gtk_widget_grab_focus( (GtkWidget*) g_object_get_data( G_OBJECT(
                akten_window), "combo_beteiligtenart" ) );
    }
...
void
aktenbetwidgets_oeffnen( GtkWidget* akten_window )
{
    Sojus* sojus = (Sojus*) g_object_get_data( G_OBJECT(akten_window), "sojus" );
    GtkWidget* grid = (GtkWidget*) g_object_get_data( G_OBJECT(akten_window), "grid" );
    ...
    gtk_container_add( GTK_CONTAINER(frame_beteiligtenart), combo_beteiligtenart );
    ...
    g_object_set_data( G_OBJECT(akten_window), "button_neue_adresse", button_neue_adresse );

    g_object_set_data( G_OBJECT(akten_window), "combo_beteiligtenart", combo_beteiligtenart );
    ...
    g_object_set_data( G_OBJECT(akten_window), "button_uebernehmen", button_uebernehmen );
    g_object_set_data( G_OBJECT(akten_window), "button_verwerfen", button_verwerfen );
    ...
    g_signal_connect( button_uebernehmen, "clicked",
            G_CALLBACK(cb_button_uebernehmen_clicked), (gpointer) akten_window );
...
void
cb_aktenbet_geaendert( gpointer window )
{
...
void
aktenbetwidgets_fuellen( GtkWidget* akten_window, Aktenbet* aktenbet )
{
    Sojus* sojus = (Sojus*) g_object_get_data( G_OBJECT(akten_window), "sojus" );
    ...
    //Adressnr
    if ( aktenbet->adressnr ) text = g_strdup_printf( "%i", aktenbet->adressnr );
    else text = g_strdup( "" );
    ...
    //Betreff1
    gtk_entry_set_text( GTK_ENTRY(g_object_get_data( G_OBJECT(akten_window),
            "entry_betreff1" )), aktenbet->betreff1 );

    //Betreff2
    gtk_entry_set_text( GTK_ENTRY(g_object_get_data( G_OBJECT(akten_window),
            "entry_betreff2" )), aktenbet->betreff2 );

    //Betreff3
    gtk_entry_set_text( GTK_ENTRY(g_object_get_data( G_OBJECT(akten_window),
            "entry_betreff3" )), aktenbet->betreff3 );

    /*
**  Speichern-Überwachung scharf schalten  */
    GtkWidget* combo_beteiligtenart = (GtkWidget*) g_object_get_data( G_OBJECT(
            akten_window), "combo_beteiligtenart" );
    g_signal_connect_swapped( combo_beteiligtenart, "changed",
            G_CALLBACK(cb_aktenbet_geaendert), (gpointer) akten_window );

    GtkEntryBuffer* entry_buffer_betreff1 = gtk_entry_get_buffer( GTK_ENTRY(
    ...
    GtkEntryBuffer* entry_buffer_betreff2 = gtk_entry_get_buffer( GTK_ENTRY(
    ...
    GtkEntryBuffer* entry_buffer_betreff3 = gtk_entry_get_buffer( GTK_ENTRY(
...
void
aktenbetwidgets_leeren( GtkWidget* akten_window )
{
    //Adressnr
    gtk_entry_set_text( GTK_ENTRY(g_object_get_data( G_OBJECT(akten_window),
            "entry_adressnr" )), "" );

    gtk_combo_box_set_active( GTK_COMBO_BOX(g_object_get_data( G_OBJECT(
            akten_window), "combo_beteiligtenart" )), -1 );

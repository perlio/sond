...
gboolean cb_window_delete_event(GtkWidget *window, GdkEvent *event,
		gpointer user_data) {
	...
	if (gtk_widget_get_sensitive(button_speichern)) {
		...
		if (rc == GTK_RESPONSE_YES)
			akte_speichern(window);
	}

	GPtrArray *arr_aktenbet = g_object_get_data(G_OBJECT(window),
			"arr_aktenbet");
	if (arr_aktenbet)
		g_ptr_array_free(arr_aktenbet, TRUE);

	return FALSE;
}

void cb_button_abbrechen_clicked(GtkButton *button, gpointer user_data) {
...
void cb_button_speichern_clicked(GtkButton *button, gpointer akten_window) {
	akte_speichern((GtkWidget*) akten_window);

	return;
}

void cb_button_ok_clicked(GtkButton *button, gpointer user_data) {
	GtkWidget *akten_window = (GtkWidget*) user_data;
	GtkWidget *button_speichern = g_object_get_data(G_OBJECT(akten_window),
			"button_speichern");
...
void cb_button_reakt_clicked(GtkButton *button, gpointer user_data) {
	gint regnr = 0;
	gint jahr = 0;
...
void cb_button_ablegen_clicked(GtkButton *button, gpointer user_data) {
	gint regnr = 0;
	gint jahr = 0;
	...
	if (gtk_widget_get_sensitive(button_speichern)) {
		gint rc = 0;
		rc = ask_question(akten_window, "Vor Ablage Änderungen speichern?",
				"Ja", "Nein");
...
void cb_button_aktenbet_aendern_clicked(GtkButton *button, gpointer user_data) {
	GtkWidget *akten_window = (GtkWidget*) user_data;
	...
	if (gtk_widget_get_sensitive(
			(GtkWidget*) g_object_get_data(G_OBJECT(akten_window),
					"button_speichern")))
		g_object_set_data(G_OBJECT(akten_window), "geaendert",
				GINT_TO_POINTER(1));
	else
		g_object_set_data(G_OBJECT(akten_window), "geaendert", NULL);
...
void cb_button_aktenbet_hinzu_clicked(GtkButton *button, gpointer user_data) {
	GtkWidget *akten_window = (GtkWidget*) user_data;
...
void cb_button_aktenbet_loeschen_clicked(GtkButton *button, gpointer user_data) {
	GtkWidget *akten_window = (GtkWidget*) user_data;
	...
	while ((index >= 0)
			&& !gtk_list_box_get_row_at_index(GTK_LIST_BOX(listbox), index))
		index--;
	if (index >= 0)
...
void cb_listbox_aktenbet_geaendert(GtkListBox *listbox, GtkListBoxRow *row,
...
void cb_akte_geaendert(gpointer window) {
	widgets_akte_geaendert((GObject*) window, TRUE);

	return;
}

void cb_button_neue_akte_clicked(GtkButton *button, gpointer user_data) {
	GtkWidget *akten_window = (GtkWidget*) user_data;
...
void cb_entry_regnr_activate(GtkEntry *entry, gpointer user_data) {
	GtkWidget *akten_window = (GtkWidget*) user_data;
	...
	if (auswahl_regnr_ist_wohlgeformt(gtk_entry_get_text(entry))) {
		...
		aktenfenster_fuellen(gtk_widget_get_toplevel(GTK_WIDGET(entry)), regnr,
				jahr);
	...
	if (ret)
		aktenfenster_fuellen(akten_window, sojus->regnr_akt, sojus->jahr_akt);
	else
...
aktenfenster_oeffnen(Sojus *sojus) {
	...
	gtk_container_add(GTK_CONTAINER(frame_sachbearbeiter_id),
	...
	GtkWidget *button_speichern = gtk_button_new_with_label("Speichern");
	...
	g_object_set_data(G_OBJECT(akten_window), "button_neue_akte",
			button_neue_akte);
	...
	g_object_set_data(G_OBJECT(akten_window), "entry_bezeichnung",
			entry_bezeichnung);
	g_object_set_data(G_OBJECT(akten_window), "entry_gegenstand",
			entry_gegenstand);
	g_object_set_data(G_OBJECT(akten_window), "combo_sachgebiete",
			combo_sachgebiete);
	...
	g_object_set_data(G_OBJECT(akten_window), "button_speichern",
			button_speichern);
	g_object_set_data(G_OBJECT(akten_window), "button_abbrechen",
			button_abbrechen);
	...
	g_signal_connect(listbox_aktenbet, "row-selected",
			G_CALLBACK(cb_listbox_aktenbet_geaendert),
			(gpointer ) akten_window);

	//button Aktenbeteiligte hinzufügen
	g_signal_connect(button_aktenbet_hinzu, "clicked",
			G_CALLBACK(cb_button_aktenbet_hinzu_clicked),
			(gpointer ) akten_window);
	...
	g_signal_connect(button_reakt, "clicked",
			G_CALLBACK(cb_button_reakt_clicked), (gpointer ) akten_window);
...
void aktenfenster_fuellen(GtkWidget *akten_window, gint regnr, gint jahr) {
	Sojus *sojus = (Sojus*) g_object_get_data(G_OBJECT(akten_window), "sojus");

	if ((regnr > 0) && (jahr > 0)
			&& auswahl_regnr_existiert(akten_window, sojus->db.con, regnr,
					jahr)) {
		GtkWidget *listbox_aktenbet =
				GTK_WIDGET(
						g_object_get_data( G_OBJECT(akten_window), "listbox_aktenbet" ));
		...
		gtk_label_set_text(
				GTK_LABEL(
						g_object_get_data( G_OBJECT(akten_window), "label_anlagedatum" )),
				text);
		g_free(text);
		...
		gtk_label_set_text(
				GTK_LABEL(
						g_object_get_data( G_OBJECT(akten_window), "label_ablagenr" )),
				text);
		g_free(text);
		...
		if (arr_aktenbet->len > 0) {
			...
			gtk_list_box_select_row(GTK_LIST_BOX(listbox_aktenbet),
					gtk_list_box_get_row_at_index(
							GTK_LIST_BOX(listbox_aktenbet), 0));
		...
		if (regnr > 0 && jahr > 0) {
			gint rc = abfrage_frage(akten_window, "Akte existiert nicht",
					"Anlegen?", NULL);
	...
	GtkEntryBuffer *entry_buffer_bezeichnung =
	...
	GtkEntryBuffer *entry_buffer_gegenstand =
			gtk_entry_get_buffer(
					GTK_ENTRY(
							g_object_get_data( G_OBJECT(akten_window), "entry_gegenstand" )));
	...
	GtkWidget *combo_sachgebiete = (GtkWidget*) g_object_get_data(
			G_OBJECT(akten_window), "combo_sachgebiete");
	g_signal_connect_swapped(combo_sachgebiete, "changed",
			G_CALLBACK(cb_akte_geaendert), (gpointer ) akten_window);

	GtkWidget *combo_sachbearbeiter = (GtkWidget*) g_object_get_data(
	...
	if (!g_strcmp0(regnr_text, "- neu -"))
		gtk_widget_set_sensitive(
				GTK_WIDGET(
						g_object_get_data( G_OBJECT(akten_window), "button_ablegen" )),
				FALSE);

	GtkListBoxRow *row = gtk_list_box_get_row_at_index(
			g_object_get_data(G_OBJECT(akten_window), "listbox_aktenbet"), 0);
	if (!row)
		widgets_aktenbeteiligte_vorhanden(G_OBJECT(akten_window), FALSE);
...
void listboxlabel_fuellen(GtkWidget *akten_window, GtkWidget *row,
		Aktenbet *aktenbet) {
	Sojus *sojus = (Sojus*) g_object_get_data(G_OBJECT(akten_window), "sojus");

...
void
aktenbet_free( gpointer user_data )
{
...
void
aktenbet_speichern( GtkWidget* akten_window )
{
    GPtrArray* arr_aktenbet = g_object_get_data( G_OBJECT(akten_window),
            "arr_aktenbet" );
    ...
    for ( gint i = 0; i < arr_aktenbet->len; i++ )
    {
        ...
        if ( (aktenbet->ID > 0) && (aktenbet->adressnr > 0) &&
                (aktenbet->geaendert) ) sql = g_strdup_printf(
                "UPDATE Aktenbet SET Adressnr=%i, "
        ...
        else if ( (aktenbet->ID > 0) && (aktenbet->adressnr == 0)) sql =
                g_strdup_printf( "DELETE FROM Aktenbet WHERE ID=%i;",
                aktenbet->ID );
        //neuer Aktenbet
        else if ( (aktenbet->ID == 0) ) sql = g_strdup_printf(
                "INSERT INTO Aktenbet (RegNr, RegJahr, "
                "Adressnr, `Beteiligtenart`, Betreff1, Betreff2, Betreff3) "
                "VALUES (%i, %i, %i, '%s', '%s', '%s', '%s');",
                sojus->regnr_akt, sojus->jahr_akt, aktenbet->adressnr, aktenbet->betart,
                aktenbet->betreff1, aktenbet->betreff2, aktenbet->betreff3 );
        ...
        if ( sql )
        {
            ...
            if ( rc )
            {
                display_message( akten_window, "Fehler bei aktenbet_speichern\n",
                        "mysql_query:\n", sql, mysql_error( sojus->db.con ), NULL );
                return;
            }
...
aktenbet_einlesen( GtkWidget* akten_window )
{
    ...
    gchar* entry_adressnr_text = (gchar*) gtk_entry_get_text(
            GTK_ENTRY(g_object_get_data( G_OBJECT(akten_window),
            "entry_adressnr" )) );
    aktenbet->adressnr = atoi( entry_adressnr_text );

    aktenbet->betart = gtk_combo_box_text_get_active_text( GTK_COMBO_BOX_TEXT(
            g_object_get_data( G_OBJECT(akten_window), "combo_beteiligtenart" )) );

    g_free( aktenbet->betreff1 );
    aktenbet->betreff1 = g_strdup( gtk_entry_get_text( GTK_ENTRY(g_object_get_data(
            G_OBJECT(akten_window), "entry_betreff1" )) ) );

    g_free( aktenbet->betreff2 );
    aktenbet->betreff2 = g_strdup( gtk_entry_get_text( GTK_ENTRY(g_object_get_data(
            G_OBJECT(akten_window), "entry_betreff2" )) ) );

    g_free( aktenbet->betreff3 );
    aktenbet->betreff3 = g_strdup( gtk_entry_get_text( GTK_ENTRY(g_object_get_data(
            G_OBJECT(akten_window), "entry_betreff3" )) ) );

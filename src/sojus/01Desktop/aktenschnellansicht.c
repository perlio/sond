...
static void cb_bu_doc_erzeugen(GtkButton *button, gpointer data) {
	...
	rc = AssocQueryString(0, ASSOCSTR_EXECUTABLE, ".odt", "open", soffice_exe,
			&bufferlen);
	if (rc != S_OK) {
		display_message(sojus->app_window, "Fehler in doc erzeugen: assoc",
				NULL);
		return;
	}
#else
	...
	argv[0] = soffice_exe;
	argv[1] =
			"C:/Users/pkrieger/AppData/Roaming/LibreOffice/4/user/template/vorlagen/Briefkopf.ott";

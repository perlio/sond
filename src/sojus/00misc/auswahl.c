...
void cb_row_activated(GtkListBox *listbox, GtkListBoxRow *row, gpointer dialog) {
...
GtkDialog*
auswahl_dialog_oeffnen(GtkWidget *window, const gchar *fenstertitel,
		const gchar *spaltentitel) {
	/*  Dialog erzeugen  */
	GtkDialog *dialog =
			GTK_DIALOG(
					gtk_dialog_new_with_buttons( fenstertitel, GTK_WINDOW(window), GTK_DIALOG_MODAL, "Ok", GTK_RESPONSE_OK, "Abbrechen", GTK_RESPONSE_CANCEL, NULL ));
...
void auswahl_dialog_zeile_einfuegen(GtkDialog *dialog, const gchar *zeile) {
...
gint auswahl_dialog_run(GtkDialog *dialog) {
	gint index = -1;
...
gboolean auswahl_regnr_existiert(GtkWidget *window, MYSQL *con, gint regnr,
		gint year) {
	gboolean ret = FALSE;
	gchar *sql = NULL;
	gint rc = 0;
...
void auswahl_parse_regnr(const gchar *entry, gint *regnr, gint *jahr) {
...
gboolean auswahl_regnr_ist_wohlgeformt(const gchar *entry) {
	if ((*entry < 48) || (*entry > 57))
		return FALSE; //erstes Zeichen muß Ziffer sein
	...
	if (strlen(g_strrstr(entry, "/")) != 3)
		return FALSE;

	return TRUE;
}

gboolean auswahl_parse_entry(GtkWidget *window, const gchar *entry, gint *regnr,
		gint *jahr) {
	...
	if (rows->len > 0) {
		GtkDialog *dialog = auswahl_dialog_oeffnen(window, "Akte auswählen",
				"   Reg.-Nr.  Aktenbezeichnung");
...
gboolean auswahl_get_regnr_akt(Sojus *sojus, GtkEntry *entry) {
	gint regnr = 0;
	gint jahr = 0;
	...
	if (!auswahl_parse_entry(sojus->app_window, entry_text, &regnr, &jahr)) {
		...
		if ((sojus->regnr_akt) && (sojus->jahr_akt))
			text = g_strdup_printf("%i/%i", sojus->regnr_akt,
					sojus->jahr_akt % 100);
		else
			text = g_strdup("");

...
static void
sojus_dir_fill_window( Sojus* sojus, GtkWidget* list_box, GArray* arr_hits )
{
...
static void
sojus_dir_cb_delete_event( GtkWidget* window, GdkEvent* event, gpointer data )
{
...
static void
sojus_dir_cb_row_activated( GtkWidget* listbox, GtkWidget* row, gpointer user_data )
{
    gint hit = 0;
    HINSTANCE ret = 0;
    gchar* path = NULL;
    ...
    hit = GPOINTER_TO_INT(g_object_get_data( G_OBJECT(row),
            "hit" ));

    //Verzeichnis öffnen
    path = g_build_filename( sojus->root, g_ptr_array_index( sojus->arr_dirs, hit ), NULL );
...
*/
    ...
    else if ( ret <= (HINSTANCE) 32 ) display_message( sojus->app_window,
            "Bei Aufruf ShellExecute:\nErrCode: %p", ret );

    g_signal_emit_by_name( gtk_widget_get_toplevel( listbox ), "delete-event", NULL, &ret_place );
...
static void
sojus_dir_show_hits( Sojus* sojus, GArray* arr_hits )
{
    //Fenster erzeugen
    GtkWidget* window = gtk_window_new( GTK_WINDOW_TOPLEVEL );
    gtk_window_set_default_size( GTK_WINDOW(window), 300, 100 );
    gtk_window_set_transient_for( GTK_WINDOW(window), GTK_WINDOW(sojus->app_window) );
    ...
    g_signal_connect( list_box, "row-activated", G_CALLBACK(sojus_dir_cb_row_activated),
            (gpointer) sojus );
    g_signal_connect( window, "delete-event", G_CALLBACK(sojus_dir_cb_delete_event), sojus );
...
sojus_dir_get_hits( Sojus* sojus, const gchar* needle )
{
    ...
    for ( gint i = 0; i < sojus->arr_dirs->len; i++ ) if ( g_str_match_string( needle,
            g_ptr_array_index( sojus->arr_dirs, i ), FALSE ) )

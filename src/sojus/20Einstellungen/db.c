 ...
 **************************************************/
static gint db_create(MYSQL *con, gchar *db_name, gchar **errmsg) {
	gint rc = 0;
	gchar *sql = NULL;
	...
	if (rc) {
		if (errmsg)
			*errmsg = g_strconcat("Fehler bei CREATE "
					"DATABASE ", db_name, ":\n", mysql_error(con),
			NULL);
	...
	if ((rc = mysql_select_db(con, db_name))) {
		if (errmsg)
			*errmsg = g_strconcat("Fehler in cb_dialog_"
...
//    rc = sond_database_add_to_database( con, errmsg );
	if (rc) {
		...
		if (ret && errmsg)
			...
			*errmsg = add_string(*errmsg,
					g_strconcat("\n\nDatabase ", db_name, " wurde gelöscht",
							NULL));
...
gint db_real_connect_database(GtkWidget *app_window, MYSQL *con, gchar **dbname) {
	do {
		gint rc = 0;
		...
		if (g_strcmp0(*dbname, "") && (*dbname)) {
			...
			if (ret)
				display_message(app_window, "Datenbank konnte nicht "
		...
		rc = dialog_with_buttons(app_window, "Datenbank auswählen", "", dbname,
				"Bestehende Datenbank", 1, "Datenbank erzeugen", 2, "Abbrechen",
		...
		if (rc == 1)
			...
			if (ret == -1) {
				...
				break;
		} else if (rc == GTK_RESPONSE_CANCEL)
			return 1;
	} while (1);
...
static gint db_get_con_params(GtkWidget *window, gchar **host, gint *port,
...
static MYSQL*
db_connect(const gchar *host, const gchar *user, const gchar *password,
		gint port, gchar **errmsg) {
...
void db_connect_database(Sojus *sojus) {
	MYSQL *con = NULL;
	...
	do {
		...
		if (try_settings)
			...
			rc = db_get_con_params(sojus->app_window, &host, &port, &user,
					&password);
		...
		if (con)
			...
			rc = dialog_with_buttons(sojus->app_window,

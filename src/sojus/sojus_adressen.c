...
static void sojus_adressen_hist_free(Hist *hist) {
	g_free(hist);

	return;
}

static void sojus_adressen_hist_row_free(HistRow *hist_row) {
	if (!hist_row)
		return;
...
static void sojus_adressen_hist_box_free(HistBox *hist_box) {
	if (!hist_box)
		return;
...
static void sojus_adressen_adresse_free(Adresse *adresse) {
...
static gboolean sojus_adressen_cb_close_request(GtkWindow *self,
...
static void sojus_adressen_cb_entry_adressnr(GtkEntry *entry, gpointer data) {
	gchar *name = NULL;
	gboolean isnum = TRUE;
	gchar *ptr = NULL;
	gchar *sql = NULL;
	...
	if (isnum) {
		...
		sql = g_strdup_printf("WITH RECURSIVE cte_labels (ID) AS ( "
				"VALUES (100) "
				"UNION ALL "
		...
		g_free(sql);
		if (rc) {
			display_message(adresse->sojus->app_window,
					"Fehler sojus_adressen_cb_entry_adressnr -\n\n",
	...
	sql = g_strdup_printf("SELECT e1.ID FROM "
			"rels JOIN "
			"entities AS e1 JOIN "
			"entities AS e2 JOIN "
			"properties "
	...
	if (rc) {
		display_message(adresse->sojus->app_window,
				"Fehler in sojus_adressen_cb_entry_adressnr -\n\n",
				"mysql_query:\n", mysql_error(adresse->sojus->con), NULL);
		return;
	}
	...
	if (arr_IDs->len > 1) //Fenster, in dem Auswahl der passenden Datensätze ermöglicht wird
		...
		adresse->sojus->adressnr_akt = g_array_index(arr_IDs, gint, 0);
	g_array_unref(arr_IDs);
...
static gboolean sojus_adressen_completion_match(GtkEntryCompletion *completion,
	...
	g_free(item);
	if (g_strstr_len(item_down, -1, key))
		ans = TRUE;
	g_free(item_down);
...
static void sojus_adressen_create_entry_completion(Sojus *sojus,
		GtkWidget *entry) {
	...
	//Model mit Liste aller Namen von Subjekten erstellen
	rc = mysql_query(sojus->con, "WITH RECURSIVE cte_labels (ID) AS ( "
			"VALUES (100) "
			"UNION ALL "
			"SELECT labels.ID FROM labels JOIN cte_labels "
			"ON labels.parent = cte_labels.ID "
	...
	gtk_entry_completion_set_match_func(completion,
			sojus_adressen_completion_match, NULL, NULL);
...
static void sojus_adressen_cb_entry_changed(GtkEntryBuffer *entry_buffer,
		gpointer data) {
	HistRow *hist_row = (HistRow*) data;

	gtk_widget_set_sensitive(hist_row->hist_box->adresse->button_speichern,
			TRUE);

	return;
}

static gpointer sojus_adressen_property_row_new(HistRow *hist_row) {
...
static void sojus_adressen_property_row_free(gpointer row) {
	g_free(row);
...
//Allgemeine Struktur
static void sojus_adressen_hist_row_delete(GtkWidget *button, gpointer data) {
	HistRow *hist_row = (HistRow*) data;
...
static void sojus_adressen_hist_row_new(HistBox *hist_box) {
	...
	gtk_box_pack_start(GTK_BOX(hist_row->hist_row),
			*((GtkWidget**) hist_row->row), FALSE,
			TRUE, 1);

	g_ptr_array_add(hist_box->hist_rows, hist_row);

	gtk_box_pack_start(GTK_BOX(hist_box->hist_box), hist_row->hist_row, FALSE,
			TRUE, 1);
...
static void sojus_adressen_cb_hist_row_new(GtkWidget *button, gpointer data) {
	HistRow *hist_row = NULL;
	...
	hist_row = g_ptr_array_index(hist_box->hist_rows,
			hist_box->hist_rows->len - 1);
...
sojus_adressen_hist_box_new(Adresse *adresse, GtkOrientation orientation,
	...
	hist_box->hist_rows = g_ptr_array_new_with_free_func(
			(GDestroyNotify) sojus_adressen_hist_row_free);
...
static void sojus_adressen_neue_adresse(Adresse *adresse) {
...
static void sojus_adressen_cb_neue_adresse(GtkWidget *button, gpointer data) {
 ...
 */
static gint sojus_adressen_speichern(Adresse *adresse, gchar **errmsg) {
	gint ret = 0;
	/*
...
static gint sojus_adressen_get_label_children(Adresse *adresse, gint label,
	...
	sql = g_strdup_printf(
			"SELECT labels.label FROM labels WHERE labels.ID = %i;", label);
	rc = mysql_query(adresse->sojus->con, sql);
	g_free(sql);
	if (rc) {
		if (errmsg)
			*errmsg = g_strconcat("Bei Aufruf mysql_query:\n",
					mysql_error(adresse->sojus->con), NULL);
	...
	sql = g_strdup_printf(
			"SELECT labels.ID FROM labels WHERE labels.parent = %i;", label);
	rc = mysql_query(adresse->sojus->con, sql);
	g_free(sql);
	if (rc) {
		if (errmsg)
	...
	for (gint i = 0; i < arr_children->len; i++) {
		rc = sojus_adressen_get_label_children(adresse,
				g_array_index(arr_children, gint, i), &iter_new, tree_store,
				errmsg);
		if (rc) {
			if (errmsg)
				*errmsg = g_strconcat("Bei Aufruf sojus_adressen_get_label:\n",
						errmsg, NULL);
			g_free(errmsg);
			g_array_unref(arr_children);
...
sojus_adressen_create_combo_subjects(Adresse *adresse, gchar **errmsg) {
	...
	rc = sojus_adressen_get_label_children(adresse, 100, NULL, tree_store,
			errmsg);
	if (rc) {
		if (errmsg)
			*errmsg = add_string(
					g_strdup("Bei Aufruf sojus_adressen_get_label_children:\n"),
					*errmsg);
...
static gint sojus_adressen_create_box(Adresse *adresse, gchar **errmsg) {
	//combobox für subject-Art
	adresse->combo_subject = sojus_adressen_create_combo_subjects(adresse,
			errmsg);
	if (!adresse->combo_subject) {
		if (errmsg)
			*errmsg = add_string(g_strdup("Bei Aufruf "
	...
	gtk_box_pack_start(GTK_BOX(adresse->box), adresse->combo_subject, FALSE,
			FALSE, 1);
...
static void sojus_adressen_cb_bu_and(GtkWidget *button, gpointer data) {
	gint rc = 0;
	gchar *errmsg = NULL;
			...
			if (rc == -1) {
				...
				message =
						g_strconcat(
								...
								errmsg, NULL);
				g_free(errmsg);
				ret = abfrage_frage(adresse->adresse, message,
						"Trotzdem neue Adresse?", NULL);
				g_free(message);
				if (ret == GTK_RESPONSE_CANCEL || ret == GTK_RESPONSE_NO)
					return;
			}
		}
	}
...
static void sojus_adressen_cb_bu_speichern(GtkWidget *button, gpointer data) {
	gint rc = 0;
	gchar *errmsg = NULL;
...
static void sojus_adressen_cb_bu_speichern_verlassen(GtkWidget *button,
		gpointer data) {
	gboolean ret = FALSE;
...
static void sojus_adressen_cb_bu_abbrechen(GtkWidget *button, gpointer data) {
	gboolean ret = FALSE;
...
void sojus_adressen_cb_fenster(GtkButton *button, gpointer data) {
	gint rc = 0;
	...
	gtk_header_bar_set_show_close_button(GTK_HEADER_BAR(adressen_headerbar),
			TRUE);
	...
	if (adresse->sojus->adressnr_akt) {
		gchar *text_adressnr = g_strdup_printf("%i",
				adresse->sojus->adressnr_akt);
...
//    gtk_widget_set_valign( outer_box, GTK_ALIGN_BASELINE );
	...
	gtk_box_pack_start(GTK_BOX(box_buttons), adresse->button_andere_adresse,
			TRUE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(box_buttons), adresse->button_speichern, TRUE,
			FALSE, 0);
	...
	sojus_adressen_create_entry_completion(adresse->sojus,
			adresse->entry_adressnr);

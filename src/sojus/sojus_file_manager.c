...
static gboolean cb_file_manager_delete_event(GtkWidget *window, GdkEvent *event,
		gpointer data) {
...
static void file_manager_cb_dir_einfuegen_p(GtkWidget *item, gpointer data) {
	gint rc = 0;
	gchar *errmsg = NULL;
...
static void file_manager_cb_dir_einfuegen_up(GtkWidget *item, gpointer data) {
	gint rc = 0;
	gchar *errmsg = NULL;
...
static void file_manager_cb_kopieren(GtkWidget *item, gpointer data) {
	SondTreeviewFM *stvfm = (SondTreeviewFM*) data;
...
static void file_manager_cb_ausschneiden(GtkWidget *item, gpointer data) {
	SondTreeviewFM *stvfm = (SondTreeviewFM*) data;
...
static void file_manager_cb_einfuegen_p(GtkWidget *item, gpointer data) {
	gint rc = 0;
	gchar *errmsg = NULL;
...
static void file_manager_cb_einfuegen_up(GtkWidget *item, gpointer data) {
	gint rc = 0;
	gchar *errmsg = NULL;
...
static void file_manager_cb_loeschen(GtkWidget *item, gpointer data) {
	gint rc = 0;
	gchar *errmsg = NULL;
			...
			g_object_get_data(
					G_OBJECT(
							sond_treeview_get_contextmenu(
									SOND_TREEVIEW(stvfm))), "item-loeschen"),
			"activate", stvfm);

	return;
}

static void file_manager_cb_eingang(GtkWidget *item, gpointer data) {
	gint rc = 0;
	gchar *errmsg = NULL;
...
static gint file_manager_open_dbase(SondTreeviewFM *stvfm, DBase **dbase,
		gchar **errmsg) {
	gint rc = 0;
	gchar *db_name = NULL;
...
//    rc = dbase_create_with_stmts( db_name, dbase, FALSE, FALSE, errmsg );
	...
	{
		if (errmsg)
			*errmsg = g_strconcat("Bei Aufruf file_manager_create_dbase:\n",
					*errmsg, NULL);
...
static void file_manager_set_headerbar(GtkWidget *fm_window,
		SondTreeviewFM *stvfm, const gchar *path) {
	...
	GtkWidget *dir_einfuegen = gtk_menu_item_new_with_label(
			"Neues Verzeichnis");
	...
	g_signal_connect(dir_einfuegen_p, "activate",
			G_CALLBACK(file_manager_cb_dir_einfuegen_p), stvfm);
	g_signal_connect(dir_einfuegen_up, "activate",
			G_CALLBACK(file_manager_cb_dir_einfuegen_up), stvfm);
	g_signal_connect(kopieren, "activate", G_CALLBACK(file_manager_cb_kopieren),
			stvfm);
	g_signal_connect(ausschneiden, "activate",
			G_CALLBACK(file_manager_cb_ausschneiden), stvfm);
	g_signal_connect(einfuegen_p, "activate",
			G_CALLBACK(file_manager_cb_einfuegen_p), stvfm);
	g_signal_connect(einfuegen_up, "activate",
			G_CALLBACK(file_manager_cb_einfuegen_up), stvfm);
...
static void file_manager_set_window(Sojus *sojus, GtkWidget *window,
		SondTreeviewFM *stvfm) {
...
file_manager_get_window(Sojus *sojus) {
	for (gint i = 0; i < sojus->arr_open_fm->len; i++) {
		...
		if (open_fm->regnr == sojus->regnr_akt
				&& open_fm->jahr == sojus->jahr_akt)
...
void file_manager_entry_activate(GtkWidget *entry, gpointer data) {/*

...
/* -------------------------------------------------------------------------
 * Index-Suche: Dialog + Ergebnisanzeige
 * ---------------------------------------------------------------------- */

static void zond_treeviewfm_index_search_row_activated(GtkTreeView *treeview,
	...
	if (!gtk_toggle_button_get_active(
			GTK_TOGGLE_BUTTON(priv->zond->fs_button)))
		gtk_toggle_button_set_active(
				GTK_TOGGLE_BUTTON(priv->zond->fs_button), TRUE);

	{
		GtkTreeIter iter_fm = { 0 };

		rc = sond_treeviewfm_file_part_visible(SOND_TREEVIEWFM(ztvfm),
		...
		if (rc == -1) {
			...
			g_clear_error(&error);
		} else if (rc == 1 && page_nr >= 0) {
			/* Datei gefunden und Seite bekannt: PDF-Viewer öffnen */
			...
			if (sfp && SOND_IS_FILE_PART_PDF(sfp)) {
				PdfPos pos_pdf = { page_nr, 0 };

				rc = zond_treeview_oeffnen_internal_viewer(priv->zond,
				...
				if (rc) {
					display_message(priv->zond->app_window,
							...
							error ? error->message : "?", NULL);
					g_clear_error(&error);
				} else if (term && priv->zond->arr_pv->len > 0) {
					/* Highlighting: letzter geöffneter PdfViewer */
					PdfViewer *pv = g_ptr_array_index(priv->zond->arr_pv,
							priv->zond->arr_pv->len - 1);
...
static void zond_treeviewfm_index_suchen_activate(GtkMenuItem *item,
		gpointer data) {
	ZondTreeviewFM *ztvfm = (ZondTreeviewFM*) data;
	ZondTreeviewFMPrivate *priv = zond_treeviewfm_get_instance_private(ztvfm);
	...
	dialog = gtk_dialog_new_with_buttons(
			"Index durchsuchen",
			GTK_WINDOW(priv->zond->app_window),
	...
	if (response == GTK_RESPONSE_OK) {
		...
		if (term && *term) {
			...
			/* Ergebnisfenster aufbauen:
			 * Spalte 0: Dateiname
			 * Spalte 1: Seite (1-basiert als String)
			 * Spalte 2: Fundstelle (snippet)
			 * Spalte 3: char_pos_in_page (versteckt, nur als String)
			 */
			...
			GtkWidget *rv = sond_result_view_new(
					...
					titel,
					cols,
					G_CALLBACK(zond_treeviewfm_index_search_row_activated),
					ztvfm);
			g_free(titel);

			/* Suchbegriff am Fenster speichern für Row-Activated-Callback */
...
void zond_treeviewfm_indiziere_leaf(SondTreeviewFM *stvfm,
...
gint zond_treeviewfm_indiziere_dir(SondTreeviewFM *stvfm,
	...
	for (guint i = 0; i < arr_children->len; i++) {
		...
		path_child = sond_tvfm_item_get_path_or_section(child);

		if (path_child) {
			/* DIR: FS-Verzeichnis, ZIP-subdir, GMessage-Multipart */
			rc = zond_treeviewfm_indiziere_dir(stvfm, child, wctx, error);
			if (rc) {
				info_window_set_message((InfoWindow*) wctx->log_func_data,
			...
			zond_treeviewfm_indiziere_leaf(stvfm,
...
static gint zond_treeviewfm_foreach_index(SondTreeview *stv, GtkTreeIter *iter,
	...
	if (sond_tvfm_item_get_path_or_section(stvfm_item)) {
		/* DIR: FS-Verzeichnis, ZIP-subdir, GMessage-Multipart */
		gint rc = zond_treeviewfm_indiziere_dir(stvfm, stvfm_item, wctx, error);
		if (rc)
			ERROR_Z
	} else
...
static void zond_treeviewfm_indiziere(SondTreeviewFM *stvfm,
	...
	if (!selection) {
		SondTVFMItem* stvfm_item_root = sond_tvfm_item_create(stvfm, NULL, NULL);
		rc = zond_treeviewfm_indiziere_dir(stvfm, stvfm_item_root, priv->zond->wctx, &error);
		g_object_unref(stvfm_item_root);
	}
	else
		rc = sond_treeview_selection_foreach(SOND_TREEVIEW(stvfm),
				zond_treeviewfm_foreach_index, priv->zond->wctx, &error);
	if (rc == -1) {
...
ZondTreeviewFM* zond_treeviewfm_new(Projekt* zond) {
	...
	//Index durchsuchen
	GtkWidget *item_index_suchen = gtk_menu_item_new_with_label(
			"Index durchsuchen");
	gtk_menu_shell_append(GTK_MENU_SHELL(contextmenu), item_index_suchen);
	...
	g_signal_connect(item_index_suchen, "activate",
			G_CALLBACK(zond_treeviewfm_index_suchen_activate),
			(gpointer) ztvfm);

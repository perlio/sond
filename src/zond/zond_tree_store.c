/*
 zond (zond_tree_store.c) - Akten, Beweisst�cke, Unterlagen
...
/* DND interfaces */
static gboolean real_zond_tree_store_row_draggable(
...
G_DEFINE_TYPE_WITH_CODE(ZondTreeStore, zond_tree_store, G_TYPE_OBJECT,
		G_ADD_PRIVATE (ZondTreeStore) G_IMPLEMENT_INTERFACE (GTK_TYPE_TREE_MODEL, zond_tree_store_tree_model_init) G_IMPLEMENT_INTERFACE (GTK_TYPE_TREE_DRAG_SOURCE, zond_tree_store_drag_source_init) G_IMPLEMENT_INTERFACE (GTK_TYPE_TREE_DRAG_DEST, zond_tree_store_drag_dest_init))

static void zond_tree_store_class_init(ZondTreeStoreClass *klass) {
...
static void zond_tree_store_init(ZondTreeStore *tree_store) {
...
static gboolean node_free(GNode *node, gpointer data) {
	...
	if (row_data->target) //ist link
	{
		...
		((RowData*) node_target->data)->links = g_list_remove(
	...
	while (list_links) {
		//link, der auf uns verweist, l�schen
		zond_tree_store_remove_node(list_links->data);
		list_links = row_data->links; //weiterspulen
	}

	//g_list_free �berfl�ssig, da alle Elemente gel�scht werden und daherlist_links NULL ist
...
static gboolean zond_tree_store_get_iter(GtkTreeModel *tree_model,
	...
	if (!zond_tree_store_iter_nth_child(tree_model, iter, &parent,
			indices[0])) {
		iter->stamp = 0;
		return FALSE;
	}
...
static GtkTreePath*
zond_tree_store_get_path(GtkTreeModel *tree_model, GtkTreeIter *iter) {
	ZondTreeStore *tree_store = (ZondTreeStore*) tree_model;
	ZondTreeStorePrivate *priv = tree_store->priv;
	GtkTreePath *retval;
	GNode *tmp_node;
	...
	g_return_val_if_fail(iter->stamp == priv->stamp, NULL); validate_tree (tree_store);
...
static gboolean zond_tree_store_iter_next(GtkTreeModel *tree_model,
	...
	g_return_val_if_fail(
			iter->stamp == ZOND_TREE_STORE (tree_model)->priv->stamp, FALSE);
 ...
 **/
static void zond_tree_store_set_changed(ZondTreeStore *tree_store, GNode *node) {
	GtkTreeIter iter = { 0 };
...
static void zond_tree_store_linked_nodes_set_changed(GNode *node) {
	GList *list = NULL;
...
void zond_tree_store_remove_node(GNode *node) {
	...
	//iter basteln, f�r path abfragen
	tree_store = ((RowData*) node->data)->tree_store;

	iter.stamp = tree_store->priv->stamp;
	iter.user_data = node;

	g_node_traverse(node, G_POST_ORDER, G_TRAVERSE_ALL, -1, node_free, NULL);
	...
	if (parent != G_NODE(tree_store->priv->root)) {
		...
		if (parent->children == NULL) {
			...
			gtk_tree_model_row_has_child_toggled(GTK_TREE_MODEL(tree_store),
					path, &new_iter);
		}
	}
	gtk_tree_path_free(path);
...
static void _do_zond_tree_store_insert(GNode *node_parent, gint pos,
	...
	//node_parent mu� zu treestore geh�ren!
...
static void zond_tree_store_insert_dummy(GNode *parent_node) {
	GtkTreeIter iter_dummy = { 0 };

	_do_zond_tree_store_insert(parent_node, 0, &iter_dummy);
	//Data-Struktur erzeugen, damit de-Referenzierung in _get nicht abst�rzt
	((RowData*) G_NODE(iter_dummy.user_data)->data)->data = g_malloc0(
			sizeof(Data));
...
static void zond_tree_store_insert_linked_nodes(GNode *node_parent, gint pos,
	...
	while (list) {
		...
		//oder link-head, aber schon geladen, oder kein link-head: volles programm
			...
			//nur einf�gen, wenn orig_new nicht schon child ist
			...
			((RowData*) orig_new->data)->links = g_list_append(
					((RowData*) orig_new->data)->links, iter_link.user_data);

			//wenn neuer Knoten Kinder hat (kommt nur bei links in Betracht):
			if (orig_new->children)
				zond_tree_store_insert_dummy(iter_link.user_data);
		}

		//when parent_link iter-head, dann k�nnen ja seinerseits links auf ihn zeigen
...
void zond_tree_store_insert(GtkTreeIter *iter, gboolean child,
	...
	if (node == tree_store->priv->root) {
		//iter zeigt auf die (unsichtbare) Wurzel des tree_store (siehe
		//zond_tree_store_iter_is_root/zond_tree_store_iter_init_root) - ganz
		//oben anhängen, unabhängig von child. Entspricht dem früher per
		//iter == NULL signalisierten Fall.
		...
		node_parent = node;
		pos = 0; //sonst funktioniert neue Anbindung einf�gen ggf. nicht!
	} else {
	...
	((RowData*) G_NODE(iter_new.user_data)->data)->data = g_malloc0(
			sizeof(Data));

	if (node_parent != tree_store->priv->root)
		zond_tree_store_insert_linked_nodes(node_parent, pos,
				iter_new.user_data);
...
static void zond_tree_store_insert_link_at_pos(GNode *node_target, gint head_nr,
	...
	((RowData*) node_target->data)->links = g_list_append(
			((RowData*) node_target->data)->links, iter_dest_new.user_data);

	//target hat Kinder? Dann Dummy-child einf�gen:
	if (node_target->children)
		zond_tree_store_insert_dummy(iter_dest_new.user_data);

	//Falls Knoten, in den link eingef�gt wird, verlinkt ist:
	if (insert_link
			&& node_parent
					!= ((RowData*) node_parent->data)->tree_store->priv->root)
		zond_tree_store_insert_linked_nodes(node_parent, pos,
...
static void zond_tree_store_load_node(GNode *node_parent,
		GNode *node_parent_target) {
	gint pos = 0;
	...
	while (node_target_child) {
		...
		if (((RowData*) node_target_child->data)->head_nr > 0)
		...
		//oder kind zeigt auf head
				...
				&& ((RowData*) ((RowData*) node_target_child->data)->target->data)->head_nr
						> 0)
		...
		} else //Kind ist kein link
		{
			...
			//in target link-Liste erg�nzen
			((RowData*) node_target_child->data)->links = g_list_append(
...
void zond_tree_store_load_link(GtkTreeIter *iter_head) {
	GNode *node_target = NULL;
	GtkTreeIter iter_dummy = { 0 };
...
static void copy_node_data(GtkTreeIter *src_iter,
		ZondTreeStore *tree_store_dest, GtkTreeIter *dest_iter) {
	GtkTreePath *path = NULL;
	RowData *data = NULL;

	data = G_NODE(dest_iter->user_data)->data;
	data->tree_store = tree_store_dest;
	data->data->icon_name = g_strdup(
			((RowData*) G_NODE(src_iter->user_data)->data)->data->icon_name);
	data->data->node_id =
			((RowData*) G_NODE(src_iter->user_data)->data)->data->node_id;
...
void zond_tree_store_copy_node(GtkTreeIter *iter_src,
	...
	zond_tree_store_insert(iter_dest, kind, iter_new);
	copy_node_data(iter_src, tree_store_dest, iter_new);
	if (zond_tree_store_iter_children(model, &child, iter_src)) {
		...
		do {
			...
			copy = *iter_new;
			zond_tree_store_copy_node(&child, tree_store_dest, &copy, kind,
					iter_new);
			kind = FALSE;
...
static void zond_tree_store_walk_tree(GNode *node, gint pos) {
	GNode *child = NULL;
	GtkTreeIter iter = { 0 };
	GtkTreePath *path = NULL;
	GtkTreeModel *model = NULL;
	...
	//wenn Knoten in link eingef�gt wirf...
...
void zond_tree_store_move_node(GtkTreeIter *iter_src,
	...
	while (list) {
		...
		if (((RowData*) link->data)->head_nr <= 0) {
			iter_link.stamp =
					((RowData*) node_src->data)->tree_store->priv->stamp;
	...
	if (child) {
		...
		if (iter_anchor)
			node_anchor = iter_anchor->user_data;
		else
			node_anchor = tree_store_anchor->priv->root;

		g_node_insert_after(node_anchor, NULL, node_src);
	} else {
		g_node_insert_after( G_NODE(iter_anchor->user_data)->parent,
				G_NODE(iter_anchor->user_data), node_src);
		pos = g_node_child_position( G_NODE(iter_anchor->user_data)->parent,
				node_src);
	}
...
gint zond_tree_store_get_link_head_nr(GtkTreeIter *iter) {
	gint head_nr = 0;
...
static void zond_tree_store_clear_traverse(GNode *node, ZondTreeStore *store) {
	GtkTreeIter iter = { 0 };
...
static gboolean zond_tree_store_drag_data_delete(GtkTreeDragSource *drag_source,
		GtkTreePath *path) {
	GtkTreeIter iter;
	if (zond_tree_store_get_iter(GTK_TREE_MODEL(drag_source), &iter, path)) {
		zond_tree_store_remove(&iter);
		return TRUE;
	} else {
		return FALSE;
...
gboolean zond_tree_store_is_link(GtkTreeIter *iter) {
...
//Gibt iter zur�ck, falls iter kein link
void zond_tree_store_get_iter_target(GtkTreeIter *iter,
...
void zond_tree_store_set_root(ZondTreeStore *tree_store, gint root_node_id) {
	tree_store->priv->root_node_id = root_node_id;

	return;
}

gint zond_tree_store_get_root(ZondTreeStore *tree_store) {
...
gboolean zond_tree_store_iter_is_root(GtkTreeIter *iter) {
	return G_NODE(iter->user_data)
			== zond_tree_store_get_root_node(zond_tree_store_get_tree_store(iter));
}

void zond_tree_store_iter_init_root(ZondTreeStore *tree_store, GtkTreeIter *iter) {
	iter->stamp = tree_store->priv->stamp;
	iter->user_data = tree_store->priv->root;

	return;
}

gint zond_tree_store_get_stamp(ZondTreeStore *tree_store) {
...
gboolean zond_tree_store_link_is_unloaded(GtkTreeIter *iter) {
	GNode *node_children = NULL;

	g_return_val_if_fail(iter, FALSE);

	if (!(node_children = G_NODE(iter->user_data)->children))
		return FALSE;

	if (((RowData*) node_children->data)->head_nr != -1)
		return FALSE;

	return TRUE;
}

gint zond_tree_store_get_node_id(GtkTreeIter *iter) {
	gint node_id = 0;
...
void zond_tree_store_kill_parent(GtkTreeIter *iter) {
	...
	while (gtk_tree_model_iter_children(model, &child, iter)) {
		...
		zond_tree_store_move_node(&child, tree_store, &anchor, FALSE,
				&inserted);

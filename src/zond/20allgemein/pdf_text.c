...
static gboolean cb_window_textsearch_delete(GtkWidget *window, GdkEvent *event,
		gpointer data) {
...
static void pdf_text_fuellen_fenster(Projekt *zond, GtkListBox *list_box,
...
static void cb_button_pdf_popover(GtkButton *button, gpointer data) {
	gtk_popover_popup(GTK_POPOVER((GtkWidget* ) data));

	return;
}

static void cb_textsuche_changed(GtkListBox *box, GtkListBoxRow *row,
		gpointer data) {
	gint index = 0;
	gint node_id = 0;
	GError *error = NULL;
	...
	GArray *arr_pdf_text_occ = g_object_get_data(G_OBJECT(box),
			"arr-pdf-text-occ");

	index = gtk_list_box_row_get_index(row);

	PDFTextOcc pdf_text_occ = g_array_index(arr_pdf_text_occ, PDFTextOcc,
			index);
	...
	if (!baum_inhalt_file)
		...
		node_id = baum_inhalt_file; //weil ja nur hierüber angebunden, nicht als Baum_inhalt_pdf_abschnitt

	//cursor dorthin setzen
	GtkTreePath *path = zond_treeview_get_path(zond->treeview[BAUM_INHALT],
			node_id);
	gtk_tree_view_expand_to_path(GTK_TREE_VIEW(zond->treeview[BAUM_INHALT]),
			path);
	gtk_tree_view_set_cursor(GTK_TREE_VIEW(zond->treeview[BAUM_INHALT]), path,
			NULL, FALSE);
...
static void cb_textsuche_act(GtkListBox *box, GtkListBoxRow *row, gpointer data) {
	Projekt *zond = (Projekt*) data;
...
pdf_text_oeffnen_fenster(Projekt *zond, GPtrArray *arr_file_part,
	...
	g_signal_connect(listbox, "row-activated", G_CALLBACK(cb_textsuche_act),
			(gpointer ) zond);
	g_signal_connect(listbox, "row-selected", G_CALLBACK(cb_textsuche_changed),
			(gpointer ) zond);
...
gint pdf_text_anzeigen_ergebnisse(Projekt *zond, gchar *search_text,
		GPtrArray *arr_file_part, GArray *arr_pdf_text_occ, gchar **errmsg) {
	GtkWidget *window = pdf_text_oeffnen_fenster(zond, arr_file_part,
			arr_pdf_text_occ, search_text);
...
static gint pdf_textsuche_pdf(Projekt *zond, const gchar *file_part,
...
fz_catch			( ctx ) {
			...
			for (block = pdf_document_page->stext_page->first_block; block;
					block = block->next) {
			...
			g_strstrip(text_tmp);
			if (*text_tmp == '\10')
				pdf_text_occ.zeile = g_strdup(text_tmp + 1);
			else
				pdf_text_occ.zeile = g_strdup(text_tmp);
			g_free(text_tmp);
...
gint pdf_textsuche(Projekt *zond, InfoWindow *info_window,
		GPtrArray *arr_file_part, const gchar *search_text,
		GArray **arr_pdf_text_occ, gchar **errmsg) {
	*arr_pdf_text_occ = g_array_new( FALSE, FALSE, sizeof(PDFTextOcc));
	g_array_set_clear_func(*arr_pdf_text_occ,

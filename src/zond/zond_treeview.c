...
static void zond_treeview_class_init(ZondTreeviewClass *klass) {
	...
	g_menu_append(sec_oeffnen, "Öffnen",        "stv.oeffnen(false)");
	g_menu_append(sec_oeffnen, "Öffnen mit",    "stv.oeffnen(true)");
 ...
 * file_part ist ein zu BAUM_FS-root relativer Pfad, ggf. gefolgt von "//"
 * und einer internen Section (Anhang/ZIP-Eintrag/Seite) - Badges werden nur
...
static SondSeadriveBadge zond_treeview_get_seadrive_badge(ZondTreeview *ztv,
	...
	if (!file_part || !ztv_priv->zond || !ztv_priv->zond->treeview[BAUM_FS])
		return SOND_SEADRIVE_BADGE_NONE;

	stvfm_fs = SOND_TREEVIEWFM(ztv_priv->zond->treeview[BAUM_FS]);
	...
	sep = g_strstr_len(file_part, -1, "//");
	rel = sep ? g_strndup(file_part, sep - file_part) : g_strdup(file_part);

	full_path = g_strconcat(root, "/", rel, NULL);
	g_free(rel);

	badge = sond_treeviewfm_seadrive_get_file_badge(stvfm_fs, full_path);
	g_free(full_path);
...
static void zond_treeview_render_icon(GtkTreeViewColumn *column,
	...
	SondSeadriveBadge seadrive_badge = SOND_SEADRIVE_BADGE_NONE;
	...
	if (zond_treeview_get_filepart_and_section(ztv, iter, &file_part, &section,
			&local_error) == -1) {
		LOG_WARN("%s: zond_treeview_get_filepart_and_section: %s", __func__,
				local_error->message);
		g_clear_error(&local_error);
	} else {
		index_status = zond_treeview_get_index_status_for_filepart(ztv,
				file_part, section);
		if (file_part)
			seadrive_badge = zond_treeview_get_seadrive_badge(ztv, file_part);
	}
	g_free(file_part);
	g_free(section);
	...
	if (index_status != SOND_INDEX_STATUS_NONE) {
		...
		if (index_pb)
			overlays[n_overlays++] =
					(SondIconOverlay) { index_pb, SOND_ICON_CORNER_BOTTOM_LEFT };
	}
	...
	if (seadrive_badge != SOND_SEADRIVE_BADGE_NONE) {
		...
		if (seadrive_pb)
			overlays[n_overlays++] =
					(SondIconOverlay) { seadrive_pb, SOND_ICON_CORNER_BOTTOM_RIGHT };
	}

	sond_icon_util_render_with_overlays(GTK_WIDGET(ztv), renderer, icon_name,
			overlays, n_overlays);
...
static gint zond_treeview_insert_file_parts(ZondTreeview *ztv, gint node_id,
	...
	iter_pdf_abschnitt = zond_tree_store_get_iter_by_node_id(
	...
	zond_tree_store_insert(iter, child, &iter_new);
...
static gint zond_treeview_leaf_anbinden(ZondTreeview *ztv,
	...
	//werden hier eingehängt. anchor_iter zeigt ggf. auf die Wurzel des
	//tree_store - das behandelt zond_tree_store_insert selbst korrekt.
	rc = zond_treeview_walk_tree(ztv, FALSE, ID_file_part,
			anchor_iter, child, &iter_new, 0,
...
static gint zond_treeview_anbinden_rekursiv(ZondTreeview *ztv,
	...
	if (sond_tvfm_item_get_item_type(stvfm_item) == SOND_TVFM_ITEM_TYPE_LEAF ||
		...
		//anchor_iter zeigt ggf. auf die (unsichtbare) Wurzel des tree_store
		//(nichts ausgewählt, siehe zond_treeview_get_anchor) - das behandelt
		//zond_tree_store_insert selbst korrekt.
		zond_tree_store_insert(anchor_iter, child, &iter_new);
		*anchor_iter = iter_new;
		*dir_inserted = TRUE;
...
gint zond_treeview_jump_to_node_id(Projekt *zond, gint node_id) {
	...
	iter = zond_tree_store_get_iter_by_node_id(
			ZOND_TREE_STORE(gtk_tree_view_get_model(
					GTK_TREE_VIEW(zond->treeview[BAUM_INHALT]))), node_id);
	if (!iter)
		return 1;
...
static gint get_anbindung_ges(Projekt *zond, gint node_id, gboolean only_anbindung,
...
static gint zond_treeview_open_auszug(ZondTreeview* ztv, GtkTreeIter* iter_parent,
	...
	do {
		...
		if (anbindung_is_pdf_punkt(anbindung_node)) {
			...
			rc = get_anbindung_ges(ztv_priv->zond, node_id, TRUE,
					anbindung_node, &anbindung_ges, error);
			if (rc) {
				g_object_unref(sfp);
...
static gint zond_treeview_open_node(Projekt *zond, GtkTreeIter *iter,
	...
	if (open_with || (sfp && !SOND_IS_FILE_PART_PDF(sfp))) {//wenn kein pdf oder mit Programmauswahl zu öffnen:
...
static void zond_treeview_row_activated(GtkTreeView *tree_view,
	...
	if (ag && g_action_group_has_action(ag, "oeffnen"))
		g_action_group_activate_action(ag, "oeffnen", g_variant_new_boolean(FALSE));
}
...
static void zond_treeview_init_contextmenu(ZondTreeview *ztv) {
	...
	struct { const gchar *name; GCallback cb; const GVariantType* type; } acts[] = {
		{ "einf-ge",       G_CALLBACK(zond_treeview_action_einf_ge),		NULL},
		{ "einf-up",       G_CALLBACK(zond_treeview_action_einf_up),		NULL},
		{ "paste-ge",      G_CALLBACK(zond_treeview_action_paste_ge),		NULL},
		{ "paste-up",      G_CALLBACK(zond_treeview_action_paste_up),		NULL},
		{ "paste-link-ge", G_CALLBACK(zond_treeview_action_paste_link_ge), 	NULL},
		{ "paste-link-up", G_CALLBACK(zond_treeview_action_paste_link_up),	NULL},
		{ "loeschen",      G_CALLBACK(zond_treeview_action_loeschen),		NULL},
	...
	for (guint i = 0; i < G_N_ELEMENTS(acts); i++) {
		GSimpleAction *act = g_simple_action_new(acts[i].name, acts[i].type);
...
static gint zond_treeview_insert_links_foreach(ZondTreeview *ztv,
	...
	if (node_id >= 0) {
		...
		iter_target = zond_tree_store_get_iter_by_node_id(
				ZOND_TREE_STORE(gtk_tree_view_get_model(
						GTK_TREE_VIEW(ztv_priv->zond->treeview[root]))),
				node_id);
		if (!iter_target) {
			if (error)
				*error = g_error_new( ZOND_ERROR, 0, "%s\nKein Iter ermittelt",
		...
		zond_tree_store_insert_link(iter_target, head_nr,
				...
				zond_tree_store_iter_is_root(&iter_anchor) ?
...
gint zond_treeview_load_baum(ZondTreeview *ztv, GError **error) {
	...
	zond_tree_store_clear(tree_store);

	root = zond_tree_store_get_root(tree_store);
	...
	//Baum ist noch leer - kein echter Anker-Iter vorhanden, also einen auf
	//die Wurzel des tree_store konstruieren (siehe zond_tree_store_insert).
	zond_tree_store_iter_init_root(tree_store, &iter_root);

	rc = zond_treeview_walk_tree(ztv, TRUE, first_child, &iter_root, TRUE, NULL,
...
static gint zond_treeview_get_filepart_and_section(ZondTreeview *ztv,
		GtkTreeIter *iter, gchar **file_part, gchar** section, GError **error) {
	gint node_id = 0;
	gint type = 0;
	gint link = 0;

...
zond_convert_get_younger_sibling_0( ZondDBase* zond_dbase, Baum baum, gint node_id, GError** error )
{
    ...
    const gchar* sql[] = {
        //...
            "SELECT inhalt2.node_id FROM old.baum_inhalt AS inhalt1 "
                "LEFT JOIN old.baum_inhalt AS inhalt2 "
                "ON inhalt1.node_id = inhalt2.older_sibling_id "
                "WHERE inhalt1.node_id = ?1;",

            "SELECT baum2.node_id FROM old.baum_auswertung AS baum1 "
                "LEFT JOIN old.baum_auswertung AS baum2 "
                "ON baum1.node_id = baum2.older_sibling_id "
                "WHERE baum1.node_id = ?1;",
        };
    ...
    if ( rc == SQLITE_ROW ) younger_sibling =
            sqlite3_column_int( stmt[baum - 1], 1 );
...
zond_convert_get_first_child_0( ZondDBase* zond_dbase, Baum baum, gint node_id, GError** error )
{
    ...
    const gchar* sql[] = {
        //...
            "SELECT inhalt1.node_id, inhalt2.node_id FROM old.baum_inhalt AS inhalt1 "
                "LEFT JOIN old.baum_inhalt AS inhalt2 "
                "ON inhalt1.node_id = inhalt2.parent_id AND inhalt2.older_sibling_id = 0 "
                    "AND inhalt2.node_id != 0 "
                "WHERE inhalt1.node_id = ?;",

            "SELECT inhalt1.node_id, inhalt2.node_id FROM old.baum_auswertung AS inhalt1 "
                "LEFT JOIN old.baum_auswertung AS inhalt2 "
                "ON inhalt1.node_id = inhalt2.parent_id AND inhalt2.older_sibling_id = 0 "
                    "AND inhalt2.node_id != 0 "
                "WHERE inhalt1.node_id = ?;"
        };
...
zond_convert_get_node_from_baum_inhalt_0( ZondDBase* zond_dbase, gint node_id, gchar** icon_name,
        gchar** node_text, gchar** rel_path, gchar** ziel_von, gint* index_von,
        gchar** ziel_bis, gint* index_bis, gint* link_id, GError** error )
{
    ...
    if ( rc == SQLITE_ROW )
    {
        ...
        *ziel_von = g_strdup( (gchar const*) sqlite3_column_text( stmt[0], 3 ) );
...
zond_convert_0_to_1_baum_inhalt_insert( ZondDBase* zond_dbase, gint anchor_id, gboolean child, gint node_id,
        gchar const* icon_name, gchar const* node_text, gchar const* rel_path, gchar const* ziel_von,
        ...
        GArray* arr_targets, gint* node_inserted, GError** error )
{
    ...
    else if ( rel_path && !ziel_von ) //datei
    {
        ...
        rc = zond_dbase_insert_node( zond_dbase, anchor_id, child, ZOND_DBASE_TYPE_BAUM_INHALT_FILE,
                0, NULL, NULL, NULL, NULL, NULL, error );
        if ( rc == -1 ) ERROR_Z

        //FILE_PART_ROOT einfügen
        rc = zond_dbase_create_file_root( zond_dbase, rel_path, icon_name, node_text, NULL, node_inserted, error );
        if ( rc ) ERROR_Z

        //neue Rekursion mit FILE_PART_ROOT starten
        first_child_file = zond_convert_get_first_child_0( zond_dbase, node_id, BAUM_INHALT, error );
        ...
        if ( first_child_file )
        {
            ...
            rc = zond_convert_0_to_1_baum_inhalt( zond_dbase, *node_inserted, TRUE, first_child_file, data_convert,
                    arr_targets, node_inserted, error );
        ...
        gint node_id, DataConvert* data_convert, GArray* arr_targets, gint* node_inserted, GError** error )
{
    ...
    rc = zond_convert_get_node_from_baum_inhalt_0( zond_dbase, node_id, &icon_name, &node_text, &rel_path,
            &ziel_von, &index_von, &ziel_bis, &index_bis, &id_link, error );
    if ( rc ) ERROR_Z

    rc = zond_convert_0_to_1_baum_inhalt_insert( zond_dbase, anchor_id, child, node_id, icon_name, node_text,
            rel_path, ziel_von, index_von, ziel_bis, index_bis, data_convert, arr_targets, node_inserted, error );

    if ( !rc && rel_path && !ziel_von ) stop_rec = TRUE;
    ...
    if ( stop_rec ) return 0; //weil ist Datei; neuer Zweig wird eingefügt
    ...
    if ( first_child > 0 )
    {
        ...
        rc = zond_convert_0_to_1_baum_inhalt( zond_dbase, *node_inserted, TRUE, first_child, data_convert, arr_targets, node_inserted, error );
        if ( rc ) ERROR_Z
    }
    ...
    if ( younger_sibling > 0 )
    {
        ...
        rc = zond_convert_0_to_1_baum_inhalt( zond_dbase, *node_inserted, FALSE, younger_sibling, data_convert, arr_targets, node_inserted, error );
        if ( rc ) ERROR_Z
    }
...
zond_convert_0_to_1( ZondDBase* zond_dbase, GError** error )
{
    ...
    if ( first_child )
    {
        ...
        rc = zond_convert_0_to_1_baum_inhalt( zond_dbase, 2, TRUE, first_child,
    ...
    if ( first_child )
    {
        ...
        rc = zond_convert_0_to_1_baum_auswertung( zond_dbase, 1, TRUE, first_child, arr_links, arr_targets, error );
        if ( rc )
        {
            g_array_unref( arr_targets );

...
static gint document_get_num_of_pages_of_dd(DisplayedDocument *dd) {
	...
	if (dd->anbindung)
		anz_seiten = dd->anbindung->bis.seite - dd->anbindung->von.seite + 1;
	else
		anz_seiten = zond_pdf_document_get_number_of_pages(
...
gint document_save_dd(DisplayedDocument *dd, gboolean ask, GError **error) {
	...
	if (arr_journal->len > 0) //es wurde etwas in diesem doc geändert
		...
		} else if (rc == GTK_RESPONSE_DELETE_EVENT)
			return 1;
	}

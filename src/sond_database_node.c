...
static void sond_database_node_class_init(SondDatabaseNodeClass *klass) {

	return;
}

static void sond_database_node_init(SondDatabaseNode *self) {
	SondDatabaseNodePrivate *priv = sond_database_node_get_instance_private(
			self);
...
sond_database_node_new(void) {
	...
	gtk_orientable_set_orientation(GTK_ORIENTABLE(sdn),
...
gint sond_database_node_load(SondDatabaseNode *sdn, gpointer database,
		gint ID_entity, gchar **errmsg) {
	...
	for (gint i = 0; i < ((arr_i_rels->len <= 10) ? arr_i_rels->len : 10);
		...
		gtk_box_pack_start(GTK_BOX(priv->box_incoming), box_segment, FALSE,
				FALSE, 0);

		ID_entity_rel = g_array_index(arr_i_rels, gint, i);

		rc = sond_database_get_subject_from_rel(database, ID_entity_rel,
				errmsg);
		if (rc == -1) {
		...
		sde_object = sond_database_entity_load_new(database, ID_entity_subject,
				errmsg);
		if (!sde_object) {
			g_array_unref(arr_i_rels);
			ERROR_Z
		...
		sde_rel = sond_database_entity_load_new(database, ID_entity_rel,
				errmsg);
		if (!sde_rel) {
			g_array_unref(arr_i_rels);
			ERROR_Z
	...
	for (gint i = 0; i < ((arr_o_rels->len <= 10) ? arr_o_rels->len : 10);
		...
		box_segment = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 10);
		gtk_box_pack_start(GTK_BOX(priv->box_outgoing), box_segment, FALSE,
				FALSE, 0);
		...
		sde_object = sond_database_entity_load_new(database, ID_entity_object,
				errmsg);
		if (!sde_object) {
			g_array_unref(arr_o_rels);
			ERROR_Z

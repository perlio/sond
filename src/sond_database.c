...
gint sond_database_begin(gpointer database, GError **error) {
	gint rc = 0;
...
gint sond_database_commit(gpointer database, GError **error) {
	gint rc = 0;
...
gint sond_database_rollback(gpointer database, GError **error) {
	gint rc = 0;
...
static gint sond_database_insert_row(gpointer database, Type type,
		gint ID_subject, gint ID_object, const gchar *prop_value,
		GError **error) {
	gint new_node_id = 0;
	...
	if (ZOND_IS_DBASE(database)) {
		...
		if (rc == SQLITE_ROW)
			new_node_id = sqlite3_column_int(stmt[1], 0);
		else {
			if (error)
				*error = g_error_new( SOND_DATABASE_ERROR,
						SOND_DATABASE_ERROR_NORESULT, "%s\n\nFehlermeldung: %s",
						__func__,
						"Ergebnis der Operation konnte nicht abgefragt werden");
			return -1;
		}
	} else //mySQL-con wird übergeben
	{
		...
		if (ID_subject)
			string_subject = g_strdup_printf("%i", ID_subject);
		else
			string_subject = g_strdup("NULL");

		if (ID_object)
			string_object = g_strdup_printf("%i", ID_object);
		else
			string_object = g_strdup("NULL");

		if (prop_value)
			prop_value_sql = g_strdup_printf("'%s'", prop_value);

		sql_1 =
				g_strdup_printf(
						"INSERT INTO entities (type, ID_subject, ID_object, prop_value) "
								"VALUES (%i,%s,%s,%s);", type, string_subject,
						string_object, (prop_value) ? prop_value_sql : "NULL");
		...
		if (rc) {
			if (error)
				*error = g_error_new(g_quark_from_static_string("MARIADB"),
						mysql_errno(database), "%s\n%s\n\nFehlermeldung: %s",
						__func__, "mysql_query (sql_2)", mysql_error(con));
			return -1;
		}
		...
		if (!mysql_res) {
			if (error)
				*error = g_error_new(g_quark_from_static_string("MARIADB"),
						mysql_errno(database), "%s\n%s\n\nFehlermeldung: %s",
						__func__, "mysql_store_result (sql_2)",
						mysql_error(con));
			return -1;
		}
		...
		if (!mysql_row) {
			if (error)
				*error = g_error_new(g_quark_from_static_string("MARIADB"),
						mysql_errno(database), "%s\n%s\n\nFehlermeldung: %s",
						__func__, "mysql_fetch_row (sql_2)", mysql_error(con));
			return -1;
		}
...
gint sond_database_insert_entity(gpointer database, Type type, GError **error) {
	gint rc = 0;
...
gint sond_database_insert_rel(gpointer database, Type type, gint ID_subject,
		gint ID_object, GError **error) {
	gint rc = 0;

	rc = sond_database_insert_row(database, type, ID_subject, ID_object, NULL,
			error);
	g_prefix_error(error, "%s\n", __func__);
...
gint sond_database_insert_property(gpointer database, Type type,
		gint ID_subject, const gchar *prop_value, GError **error) {
	gint rc = 0;

	rc = sond_database_insert_row(database, type, ID_subject, 0, prop_value,
			error);
	g_prefix_error(error, "%s\n", __func__);
...
sond_database_get_properties(gpointer database, gint ID_subject, GError **error) {
	...
	if (ZOND_IS_DBASE(database)) {
		...
		g_array_set_clear_func(arr_properties,
				(GDestroyNotify) sond_database_clear_property);
		do {
			Property property = { 0 };
			...
			property.value = g_strdup(
					(const gchar* ) sqlite3_column_text(stmt[0], 2));
...
static gint sond_database_compare_property_value(gconstpointer a,
		gconstpointer b) {
...
GArray*
sond_database_get_properties_of_type(gpointer database, gint type,
		gint ID_subject, GError **error) {
	GArray *arr_properties = NULL;
	...
	if (ZOND_IS_DBASE(database)) {
		...
		do {
			...
			property.ID_subject = ID_subject;
			property.value = g_strdup(
					(const gchar* ) sqlite3_column_text(stmt[0], 1));
			g_array_append_val(arr_properties, property);
		} while (rc == SQLITE_ROW);
	} else //mysql
	{
		...
		sql_mariadb = g_strdup_printf("SET @1=%i; SET @2=%i; ", type,
				ID_subject);
		sql_mariadb = add_string(sql_mariadb, g_strdup(sql[0]));
...
gint sond_database_get_only_property_of_type(gpointer database, gint type,
		gint ID_subject, Property *property, GError **error) {
	GArray *arr_properties = NULL;

	arr_properties = sond_database_get_properties_of_type(database, type,
			ID_subject, error);
	if (!arr_properties) {
		g_prefix_error(error, "%s\n", __func__);
	...
	if (arr_properties->len != 1) {
		if (error) {
			...
			if (arr_properties->len == 0)
				code = SOND_DATABASE_ERROR_NORESULT;
			else
				code = SOND_DATABASE_ERROR_TOOMANYRESULTS;
...
gint sond_database_label_is_equal_or_parent(gpointer database,
	...
	if (ZOND_IS_DBASE(database)) {
		...
		if (rc != SQLITE_OK)
			ERROR_ZOND_DBASE("sqlite3_bind_int (ID_label_parent)")

		rc = sqlite3_bind_int(stmt[0], 2, ID_label_test);
		if (rc != SQLITE_OK)
			ERROR_ZOND_DBASE("sqlite3_bind_int (ID_label_test)")
...
gint sond_database_get_ID_label_for_entity(gpointer database, gint ID_entity,
	...
	if (ZOND_IS_DBASE(database)) {
		...
		if (rc != SQLITE_ROW && rc != SQLITE_DONE)
			...
			ERROR_S_MESSAGE("ID_entity existiert nicht")
...
static gint sond_database_get_object_for_subject_one_step(gpointer database,
	...
	if (ZOND_IS_DBASE(database)) {
		...
		if (rc != SQLITE_OK)
			ERROR_ZOND_DBASE("sqlite3_bind_int (ID_label_object)")

		rc = sqlite3_bind_int(stmt[0], 2, ID_entity_subject);
		if (rc != SQLITE_OK)
			ERROR_ZOND_DBASE("sqlite3_bind_int (ID_entity_subject)")

		rc = sqlite3_bind_int(stmt[0], 3, ID_label_rel);
		if (rc != SQLITE_OK)
			ERROR_ZOND_DBASE("sqlite3_bind_int (ID_label_rel)")
...
gint sond_database_get_entities_for_property(gpointer database,
	...
	if (ZOND_IS_DBASE(database)) {
		...
		if (rc != SQLITE_OK)
			ERROR_ZOND_DBASE("sqlite3_bind_int (ID_label_property)")

		rc = sqlite3_bind_text(stmt[0], 2, value, -1, NULL);
		if (rc != SQLITE_OK)
			ERROR_ZOND_DBASE("sqlite3_bind_text (value)")
		...
		do {
			...
			if (rc != SQLITE_ROW && rc != SQLITE_DONE) {
				...
				ID_entity = sqlite3_column_int(stmt[0], 0);
				if (arr_res)
					g_array_append_val(*arr_res, ID_entity);
				cnt++;
			}
		} while (rc == SQLITE_ROW);
	} else //mysql
...
gint sond_database_get_property_value(gpointer database, gint ID_property,
	...
	if (ZOND_IS_DBASE(database)) {
		...
		if (rc != SQLITE_OK)
			ERROR_ZOND_DBASE("sqlite3_bind_int (ID_property)")
...
gint sond_database_get_first_property_value_for_subject(gpointer database,
	...
	} else //mysql
	{
		...
		sql_mariadb = add_string(sql_mariadb,
				g_strdup_printf("SET @2=%i; ", ID_label_property));
		sql_mariadb = add_string(sql_mariadb, g_strdup(sql[0]));
...
gint sond_database_get_subject_and_first_property_value_for_labels(
	...
	if (ZOND_IS_DBASE(database)) {
		...
		if (rc != SQLITE_OK)
			ERROR_ZOND_DBASE("sqlite3_bind_int (ID_label_subject)")
...
gint sond_database_get_subject_from_rel(gpointer database, gint ID_entity_rel,
	...
	if (ZOND_IS_DBASE(database)) {
		...
		if (rc != SQLITE_OK)
			ERROR_ZOND_DBASE("sqlite3_bind_int (ID_entity_rel)")

		rc = sqlite3_step(stmt[0]);
		if (rc != SQLITE_ROW && rc != SQLITE_DONE)
			ERROR_ZOND_DBASE("sqlite3_step")
		else if (rc == SQLITE_DONE)
			ERROR_S_MESSAGE("Rel nicht vorhanden");

...
G_DEFINE_TYPE_WITH_PRIVATE(SondDatabaseProperty, sond_database_property, SOND_TYPE_DATABASE_ENTITY)
...
static void
sond_database_property_init( SondDatabaseProperty* self )
{
    GtkWidget* frame_value = NULL;

    SondDatabasePropertyPrivate* priv = sond_database_property_get_instance_private( self );
...
sond_database_property_new( void )
{
    ...
    gtk_orientable_set_orientation( GTK_ORIENTABLE(sdp), GTK_ORIENTATION_VERTICAL );
...
sond_database_property_load_new( gpointer database, gint ID_property, gchar** errmsg )
{
    SondDatabaseProperty* sdp = NULL;
    ...
    rc = sond_database_entity_load( SOND_DATABASE_ENTITY(sdp), database, ID_property, errmsg );
    if ( rc )
    {
        g_object_unref( sdp );
        ERROR_S_VAL( NULL )
    ...
    rc = sond_database_get_property_value( database, ID_property, &value, errmsg );
    if ( rc )
    {
        g_object_unref( sdp );
        ERROR_S_VAL( NULL )
...
void
sond_database_property_set_editable( SondDatabaseProperty* sdp, gboolean editable )
{
    SondDatabasePropertyPrivate* priv = sond_database_property_get_instance_private( sdp );

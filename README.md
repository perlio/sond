...
Das Hauptfenster gliedert sich in eine Titelleiste (Headerbar) mit Menü, drei nebeneinander angeordnete Baumansichten (Dateiverzeichnis, Bestandsverzeichnis, Auswertungsverzeichnis) sowie ein Notizfeld für Freitext zum jeweils markierten Punkt. Es öffnet sich beim Start eingerastet auf der linken Bildschirmhälfte (halbe Bildschirmbreite, volle Arbeitsflächenhöhe).
...
**Neu** legt ein neues Projekt an. War bereits ein Projekt geöffnet, wird zunächst nachgefragt, ob dieses geschlossen werden soll. Anschließend öffnet sich ein Dateiauswahl-Dialog ("Projekt anlegen"), in dem Verzeichnis und Dateiname gemeinsam bestimmt werden; das Namensfeld ist mit ".ZND" vorbelegt, der eigentliche Projektname muss davor eingetippt werden. Es werden zwei SQLite-Datenbanken angelegt: die eigentliche Projektdatei sowie eine gleichnamige Arbeitskopie mit der Endung ".tmp". Sämtliche Änderungen - Baumstruktur, Icons, Zellentext, Notizen - werden während der Arbeit sofort in diese Arbeitskopie geschrieben (beide Bäume sind also uneingeschränkt und sofort editierbar); in die eigentliche Projektdatei gelangen sie erst bei "Speichern" bzw. beim automatischen Speichern, falls aktiviert (s.u.).

**Öffnen** fragt bei ungespeichertem, geöffnetem Projekt ebenfalls zunächst nach dem Schließen und öffnet dann einen Datei-öffnen-Dialog.
...
Die Funktionen zur Volltextindizierung sind im Hauptmenü unter "Projekt > Index" in einem eigenen Untermenü mit den Punkten "Erstellen", "Durchsuchen" und "Löschen" zusammengefasst; jeder dieser Punkte bietet wiederum "Gesamtes Projekt" (wirkt immer auf das komplette Projektverzeichnis) und "Auswahl" (wirkt auf die aktuell markierten Punkte). Aus den Kontextmenüs des Dateiverzeichnisses sowie des Bestands- und Auswertungsverzeichnisses sind dieselben drei Funktionen zusätzlich direkt erreichbar, dort jedoch bewusst nur für die Auswahl (ein Rechtsklick liefert keinen sinnvollen Bezug zum gesamten Projekt). Der Indizierungsstand wird in allen drei Bäumen durch ein kleines Kreis-Icon unten links am Datei- bzw. Ordner-Icon angezeigt: grün für vollständig indiziert, grau für teilweise bzw. gemischt indiziert, kein Icon für nicht indiziert.

**Index erstellen** fragt zunächst den OCR-Modus ab: kein OCR, vorhandenen versteckten Text prüfen und bei Bedarf ergänzen (Standardauswahl), oder vorhandenen versteckten Text löschen und die Seite neu erkennen. Die Indizierung läuft danach im Hintergrund; ein Info-Fenster zeigt den Fortschritt und erlaubt den Abbruch. Ist unter den ausgewählten Punkten eine Anbindung, die nicht an einer Seitengrenze beginnt oder endet (z.B. nur die untere Hälfte einer Seite), erscheint statt dessen eine Fehlermeldung, da die Indizierung stets die ganze Seite erfasst.

**Index durchsuchen** prüft bei einer Auswahl zunächst, ob alle betroffenen Seiten bereits vollständig indiziert sind; bei Lücken erscheint eine Liste der unvollständigen Dateien - wo bekannt mit der genauen Angabe "X von Y Seiten fehlen", sonst nur mit der Anzahl bereits indizierter Seiten - mit den Optionen "Jetzt nachindizieren", "Trotzdem suchen" oder "Abbrechen". Die eigentliche Suchmaske enthält ein Feld "Suchbegriff:" sowie ein optionales Feld "Im Kontext von:"; ist Letzteres gefüllt, müssen beide Begriffe im selben Textabschnitt vorkommen (logisches UND, keine weiteren Operatoren wählbar). Enthält der Suchbegriff ein Leerzeichen, wird er automatisch als zusammenhängende Wortfolge gesucht. Ergänzend wird auch nach passenden Dateinamen gesucht. Treffer erscheinen in einer Ergebnisliste (Datei/Seite/Fundstelle); Doppelklick springt an die Fundstelle im internen Viewer bzw. in der jeweiligen Dateivorschau. Aus dem Ergebnisfenster heraus lassen sich markierte Treffer als Kopie in das Auswertungsverzeichnis übernehmen, sofern dieses gerade aktiv ist.

**Index löschen** entfernt gezielt wieder Index-Daten, etwa um eine fehlerhafte Texterkennung rückgängig zu machen oder Platz zu sparen. Betroffen sind ausschließlich die Index-Daten selbst; weder die Dateien noch im Bestands- oder Auswertungsverzeichnis angelegte Anbindungen werden dabei verändert oder gelöscht - diese verlieren lediglich ihren grünen/grauen Indizierungs-Badge und liefern in der Indexsuche keine Treffer mehr, bis neu indiziert wird. Vor dem eigentlichen Löschen wird stets zurückgefragt, da bereits geleistete OCR- und Indizierungsarbeit dabei verloren geht. Wie bei "Index erstellen" ist die Auswahl einer nicht seitengenauen Anbindung nicht zulässig.

## Baumansichten – Gemeinsamkeiten

Alle drei Bäume (Dateiverzeichnis, Bestandsverzeichnis, Auswertungsverzeichnis) beruhen auf derselben Basiskomponente und teilen sich daher folgendes Verhalten:
...
- Der Zeilentext ist per Inline-Bearbeitung änderbar: ein einfacher Klick auf den Text der bereits selektierten (fokussierten) Zeile startet die Bearbeitung. Icon und Text stehen als getrennte Zellen in derselben Spalte; nur ein Doppelklick auf das Icon löst "Öffnen" aus (s.u.), da nur dort keine Bearbeitung ansteht. Im Bestands- und Auswertungsverzeichnis wird die Textänderung sofort in die Arbeitskopie (".tmp") gespeichert (s. "Projekt → Neu" oben).
...
- Bei fokussierter Baumansicht öffnet das Tippen einer beliebigen druckbaren Taste (ohne Strg) automatisch ein Suchfeld (Popover), mit dem sich der Baum nach dem eingegebenen Text durchsuchen läßt (s. "Suchen" unten).
- Im Bestands- und Auswertungsverzeichnis werden Zeilen, denen bereits ein Notiztext hinterlegt ist, farblich hervorgehoben.

Folgende Bearbeiten-Funktionen gibt es prinzipiell in allen drei Bäumen, mit je nach Baum unterschiedlicher Wirkung (Einzelheiten in den jeweiligen Abschnitten unten):

- **Kopieren** und **Ausschneiden** merken sich zunächst nur die markierten Zeilen (Referenz), ohne etwas zu verändern; ausgeschnittene Zeilen werden ausgegraut dargestellt.
- **Einfügen** (gleiche Ebene / Unterebene) wird erst dadurch wirksam. Der Zielknoten darf dabei nie ein Abkömmling der zu verschiebenden/kopierenden Auswahl sein. Wird innerhalb desselben Baums ausgeschnitten und eingefügt, wird der Knoten in jedem Baum tatsächlich verschoben; was Einfügen einer Kopie bzw. das Einfügen aus einem anderen Baum bewirkt, unterscheidet sich dagegen von Baum zu Baum erheblich (s.u.).
- **Öffnen** (Doppelklick auf das Icon einer Zeile, Menüpunkt "Öffnen"/"Öffnen mit", Tastenkürzel Strg+O) zeigt PDF-Dateien stets im internen Viewer an; bestimmte weitere Formate (Text, Bilder, ODT, DOCX, E-Mail-Bestandteile) werden über "Öffnen" ebenfalls immer intern angezeigt, "Öffnen mit" umgeht diese interne Anzeige. Andere Dateien werden mit der Standardanwendung des Systems geöffnet; "Öffnen mit" öffnet dafür unter Windows den systemeigenen Auswahldialog für die Anwendung. In Containern (ZIP, E-Mail, PDF) eingebettete Dateien werden dafür zunächst in eine temporäre Datei extrahiert. Was genau als PDF-Ausschnitt angezeigt wird und welche Wirkung dabei die Tasten Strg, Alt und Umschalt haben, hängt vom jeweiligen Baum ab und ist unten bei den einzelnen Bäumen beschrieben; Wird beim Doppelklick zusätzlich die Umschalttaste gehalten, öffnet sich in jedem Baum stets ein neues Viewer-Fenster, statt ein bereits offenes (das denselben Inhalt zeigt) wiederzuverwenden.

**Löschen** entfernt den markierten Knoten **ohne Sicherheitsabfrage** - in allen drei Bäumen, im Dateiverzeichnis u.a. auch für einzelne Dateien innerhalb eines ZIP-Archivs oder von MIME-Teilen (E-Mail-Anhängen). Bei reinen Strukturpunkten im Bestandsverzeichnis werden zuvor alle Unterpunkte rekursiv mitgelöscht. Ist der Punkt oder die zugehörige Datei an anderer Stelle (Auswertungsverzeichnis-Kopie) noch in Verwendung, wird das Löschen für diesen Knoten übersprungen, um verwaiste Kopien zu vermeiden. Ein Link, der nicht der Kopf ist, wird beim Löschen ignoriert.

Im Bestands- und Auswertungsverzeichnis stehen zusätzlich folgende, dort identisch wirkende Funktionen zur Verfügung (im Dateiverzeichnis nicht vorhanden):
...
**Suchen** öffnet ein Suchfeld (s.o.); ab drei eingegebenen Zeichen und Enter wird gesucht. Durchsucht werden gleichzeitig Dateipfad, Zeilentext **und der Notiztext** aller Punkte, nicht nur der sichtbare Baumtext. Treffer erscheinen in einer eigenen Ergebnisliste; Doppelklick springt zur Fundstelle.

**Icon ändern** weist allen markierten Zeilen ohne weitere Rückfrage eines der folgenden Icons zu: Punkt, Ordner, Datei, PDF-Datei, PDF, Anbindung, Akte, Ausführbar, Text, Writer/Word, PowerPoint, Tabelle, Bild, Video, Audio, E-Mail, HTML, Durchsuchung, Ort, TKÜ, Wichtig, Observation, CD, Person, Personen sowie die Farbmarkierungen Orange, Blau, Rot, Grün, Türkis und Magenta.
...
| Strg+O | Öffnen |
| Umschalt (beim Öffnen aus dem Baum) | stets neues Viewer-Fenster öffnen, statt ein bereits offenes wiederzuverwenden |
| Strg (beim Öffnen aus dem Baum) | nur die angeklickte Anbindung selbst anzeigen (Einzelheiten je Baum s.u.) |
| Alt (beim Öffnen aus dem Baum) | an das Ende statt an den Anfang des angezeigten Abschnitts springen |

Weitere feste Interaktion ohne Menü-Accelerator: eine beliebige druckbare Taste öffnet das Such-Popover (s.o.).

## Dateiverzeichnis

Zeigt den Inhalt des Projektverzeichnisses als Dateibaum (s. "Design" oben); zu PDF-Dateien werden zusätzlich die dort bereits definierten Abschnitte angezeigt. Im Kontextmenü stehen die üblichen Operationen auf Dateisystemebene zur Verfügung (kopieren, löschen, verschieben, weitere Unterverzeichnisse anlegen), außerdem - unter Windows - ein Abschnitt zur SeaDrive-Cloud-Synchronisation: Dateien/Verzeichnisse können als "immer offline verfügbar" markiert, diese Markierung wieder aufgehoben oder der lokale Cache geleert werden – jeweils für das gesamte Projektverzeichnis oder nur die Auswahl.

Von den oben beschriebenen gemeinsamen Bearbeiten-Funktionen wirken **Kopieren**, **Ausschneiden** und **Einfügen** im Dateiverzeichnis - anders als im Bestands- und Auswertungsverzeichnis - in den meisten Fällen als gewöhnliche Dateisystem-Operationen (Datei bzw. Verzeichnis kopieren oder verschieben). Einfügen in eine ZIP-Datei ist dabei allerdings noch nicht implementiert.

Unabhängig davon können aus dem Dateiverzeichnis kopierte oder ausgeschnittene Dateien/Verzeichnisse auch in das Bestands- oder Auswertungsverzeichnis eingefügt werden; dort werden sie dann "angebunden" (neuer Anbindungsknoten je Datei, bei Verzeichnissen rekursiv mit Strukturpunkten je Unterordner); bereits angebundene Dateien werden dabei übersprungen und im Info-Fenster gemeldet.

**PDF reparieren** wendet auf alle im Dateiverzeichnis (oder einem der beiden anderen Bäume, sofern dort gerade aktiv) markierten PDF-Dateien - ohne weitere Rückfrage - einen Strukturneuaufbau an, der von MuPDF stammende, aber fehlerhafte oder verwaiste interne Objekte bereinigt; die Seitenreihenfolge bleibt dabei unverändert. Die Originaldatei wird direkt überschrieben. Ist eine der markierten Dateien bereits in einem Viewer-Fenster geöffnet, wird die Reparatur für diese Datei mit einer Fehlermeldung abgelehnt. Bei Erfolg erscheint keine Bestätigung, nur Fehler werden gemeldet.

**Öffnen**: Bei einer PDF-Datei wird stets die gesamte Datei angezeigt; mit gedrückter Strg-Taste wird stattdessen nur der angeklickte Abschnitt selbst angezeigt (ist ein Abschnitt bereits selbst angeklickt, springt der Viewer ohne Strg an dessen Anfang bzw. mit gedrückter Alt-Taste an dessen Ende).
...
**Einfügen** bindet aus dem Dateiverzeichnis kopierte/ausgeschnittene Dateien bzw. Verzeichnisse an (s.o.). Wird dagegen eine im Bestandsverzeichnis selbst kopierte (nicht ausgeschnittene) Auswahl eingefügt, geschieht nichts - hier gilt die Regel, dass jede Datei/jeder Abschnitt nur einmal im Bestandsverzeichnis vorkommen darf. Ausschneiden und Einfügen innerhalb des Bestandsverzeichnisses verschiebt den Knoten wie gewohnt.

**Als Link einfügen** ist im Bestandsverzeichnis wirkungslos (nur im Auswertungsverzeichnis nutzbar, s.u.).

**Anbindung entfernen** unterscheidet sich von "Löschen": Es wird nur der Anbindungspunkt selbst entfernt, seine Unterpunkte bleiben erhalten und rücken eine Ebene auf.

**Öffnen**: Es wird stets nur die am angeklickten Punkt angebundene Datei bzw. der angebundene Abschnitt selbst angezeigt. Standardmäßig wird dabei der Umfang der übergeordneten, registrierten Anbindung angezeigt, sofern der angeklickte Punkt ein Unterabschnitt einer größeren Anbindung ist; mit gedrückter Strg-Taste wird stattdessen nur der angeklickte Abschnitt selbst angezeigt. Mit Alt wird an das Ende statt an den Anfang gesprungen.

## Auswertungsverzeichnis

**Punkt einfügen** legt wie im Bestandsverzeichnis einen neuen Strukturpunkt an (s.o.); die Beschränkungen des Bestandsverzeichnisses gelten hier nicht.

**Einfügen** einer im Auswertungsverzeichnis selbst kopierten Auswahl erzeugt hier - anders als im Bestandsverzeichnis - einen echten neuen Knoten (rekursiv inkl. Unterpunkte); es entsteht also eine unabhängige zweite Kopie. Ausschneiden und Einfügen verschiebt wie gewohnt den Knoten.

**Als Link einfügen** (gleiche Ebene / Unterebene) legt keine Kopie, sondern eine Referenz auf den Originalknoten an (im Auswertungs- oder Bestandsverzeichnis); Original und Link bleiben dadurch synchron – Änderungen am Original (Text, Unterpunkte) erscheinen auch am Link. Verweist die Quelle bereits selbst auf einen Link, wird direkt auf dessen Ziel verwiesen, damit keine Ketten entstehen. Ausschneiden und "Als Link einfügen" lassen sich nicht kombinieren. Verlinkte Punkte werden kursiv dargestellt, der "Kopf" eines Links (das Original des Links, an dem gelöscht/verschoben werden kann) zusätzlich violett.

**Öffnen**: Da Punkte hier frei angeordnet und verschachtelt werden können, unabhängig von der tatsächlichen Gliederung einer PDF-Datei, gilt eine eigene Logik (bei einem Link wird zunächst dessen Ziel betrachtet):

- Ist das Ziel ein Strukturpunkt, werden dessen unmittelbare Kinder - soweit PDFs oder Abschnitte - zu einem zusammenhängenden "virtuellen PDF" (Auszug) zusammengefaßt und geöffnet.
- Ist das Ziel selbst eine Anbindung (Datei oder Abschnitt), kommt es auf den Elternknoten der angeklickten Stelle an (nicht auf den des Linkziels): Ist dieser ein Strukturpunkt, wird ebenfalls ein Auszug aus dessen Kindern geöffnet - die angeklickte Anbindung erscheint darin als ein Teil unter mehreren. Ist der Elternknoten dagegen selbst eine Anbindung, oder gibt es keinen Elternknoten (oberste Ebene), wird nur die Zielanbindung geöffnet - wie bei einem Klick im Bestandsverzeichnis, einschließlich Strg-Verhalten.
- Mit gedrückter Strg-Taste wird die Auszug-Gruppierung generell übergangen, unabhängig vom Elternknoten: es wird immer nur die angeklickte Anbindung selbst geöffnet.
- Mit Alt wird an das Ende statt an den Anfang des angezeigten (Teil-)Abschnitts gesprungen.
...
Der in zond integrierte PDF-Viewer basiert auf MuPDF (nicht Poppler). Er zeigt den zu einem Punkt des Dateiverzeichnisses, Bestands- oder Auswertungsverzeichnisses gehörenden Abschnitt einer PDF-Datei an (s. "Öffnen" bei den jeweiligen Bäumen oben) und dient zugleich dazu, neue Abschnitte innerhalb einer PDF-Datei zu definieren.
...
- **OCR** erkennt für die abgefragte Seitenauswahl (hier inkl. Option "alle") unsichtbaren Text mittels Tesseract fest auf Deutsch (keine Sprachauswahl) und schreibt ihn in die PDF-Seite ein. Anders als bei "Index erstellen" (s.o.) ist hier fest der Standardmodus eingestellt: Seiten, die bereits versteckten Text enthalten, werden übergangen; eine Auswahl des OCR-Modus gibt es an dieser Stelle (noch) nicht. Ein Info-Fenster mit fortlaufendem Protokoll und Abbrechen-Knopf zeigt den Fortschritt. Bei Abbruch bleiben bereits verarbeitete Seiten erhalten, die restlichen werden nicht mehr bearbeitet. *(Geplant: Wahlmöglichkeit zwischen komplett neuem OCR-Durchlauf und einer Gegenüberstellung von vorhandenem und neu erkanntem Tesseract-Text zur Auswahl.)*
...
| Alt (beim Sprung zu einer Fundstelle der Volltextsuche) | Zielposition eine Bildschirmseite höher anzeigen |

Die Wirkung von Umschalt, Strg und Alt beim Öffnen aus einem der drei Bäume ist oben unter "Hauptfenster" beschrieben.
